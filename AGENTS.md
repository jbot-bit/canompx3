...
- `docs/plans/` — durable decisions and design docs. Cross-tool truth lives here.
- `AGENTS.md` — this file. Vendor-neutral operating rules.

**Tool-specific state (private to each tool):**
...
- WSL (Codex): uses `.venv-wsl/`
- Same repo, same lockfile (`uv.lock`). Rebuild with `uv sync --frozen`.
- Do not cross-wire envs. Windows shells never activate `.venv-wsl/`, WSL shells never activate `.venv/`.
