...
def main() -> int:
    ...
    parts.append("")
    parts.append(
        "`CONTINUE` as a bounded shortlist for conditional-role translation. "
        "The recovered winners are alive as research-level allocator / filter candidates, "
        "but this report alone is not a validated-shelf or live-routing promotion."
    )
    parts.append("")

    parts.append("## Reproduction")
    parts.append("")
    parts.append(f"- `python3 -m py_compile {Path(__file__).relative_to(ROOT)}`")
    parts.append(f"- `./.venv-wsl/bin/python {Path(__file__).relative_to(ROOT)}`")
    parts.append("")

    parts.append("## Caveats")
    parts.append("")
    parts.append("- Latest canonical day in this replay is fixed by the current DB state, not by this script.")
    parts.append("- OOS remains thin and should not be used to re-specify the shortlist.")
    parts.append("- Current repo allocation consumes validated standalone lanes, not these conditional-role outputs directly.")
    parts.append("")
