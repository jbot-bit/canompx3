...
Scope is locked by:
  docs/audit/hypotheses/2026-04-22-mnq-comex-settle-f6-inside-pdr-v1.yaml

# e2-lookahead-policy: tainted
# orb_COMEX_SETTLE_break_dir = 'long' used as a WHERE predicate (line ~108) to
...
the locked MNQ COMEX_SETTLE O5 E2 CB1 RR1.0 long lane using canonical
orb_outcomes + daily_features joins.

Outputs:
  - docs/audit/results/2026-04-22-mnq-comex-settle-f6-inside-pdr-v1.md
  - docs/audit/results/2026-04-22-mnq-comex-settle-f6-inside-pdr-v1-rows.csv
"""
...
PREREG_PATH = "docs/audit/hypotheses/2026-04-22-mnq-comex-settle-f6-inside-pdr-v1.yaml"
RESULT_DOC = Path("docs/audit/results/2026-04-22-mnq-comex-settle-f6-inside-pdr-v1.md")
ROW_CSV = Path("docs/audit/results/2026-04-22-mnq-comex-settle-f6-inside-pdr-v1-rows.csv")
...
def load_lane_df(instrument: str = "MNQ") -> pd.DataFrame:
    ...
    try:
        ...
        SELECT
            ...
            d.orb_COMEX_SETTLE_high,
            d.orb_COMEX_SETTLE_low
        FROM orb_outcomes o
    ...
    df["orb_mid"] = (df["orb_COMEX_SETTLE_high"].astype(float) + df["orb_COMEX_SETTLE_low"].astype(float)) / 2.0
    df["feature_fire"] = (
        (df["orb_mid"] > df["prev_day_low"].astype(float)) & (df["orb_mid"] < df["prev_day_high"].astype(float))
    ).astype(int)
...
def _t7_per_year_delta(df: pd.DataFrame) -> tuple[int, int, dict[int, float]]:
    ...
    for year in range(IS_START_YEAR, IS_END_YEAR + 1):
        ...
        if value < 0:
            negative += 1
    return negative, testable, year_delta
...
def main() -> int:
    ...
    if welch_p >= 0.05 or welch_t > -3.0 or observed_delta > -0.10 or negative_years < 5:
        verdict = "KILL"
    ...
    lines.extend(
        [
            ...
            f"- Negative IS years (delta): `{negative_years}` / `{testable_years}`",
            ...
            (
                ...
                f"{'PASS' if max_corr < 0.30 else 'FAIL'} | correlations={corrs} |"
            ),
            (
            ...
            (
                ...
                f"{'PASS' if off_resolved_mean > 0 and off_all_mean > 0 else 'FAIL'} | off-signal baseline remains positive |"
            ),
            (
            ...
            (
                f"| T7_per_year_delta | {negative_years}/{testable_years} negative | "
                f"{'PASS' if negative_years >= 5 else 'FAIL'} | yr_delta={year_delta} |"
            ),
            (
                f"| T8_scratch_bias | on={_fmt(scratch_rate_on)} off={_fmt(scratch_rate_off)} | "
                f"{'INFO' if scratch_rate_on > scratch_rate_off else 'PASS'} | scratch concentrations disclosed, not hidden |"
            ),
            "",
            "## Decision notes",
            "",
