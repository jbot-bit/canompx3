"""Phase D D4 — MNQ COMEX_SETTLE O5 RR1.0 long + garch_forecast_vol_pct > 70 — Pathway B K=1.

Runs the pre-registered confirmatory test per
docs/audit/hypotheses/2026-04-28-mnq-comex-settle-garch-pathway-b-v1.yaml.

Output: docs/audit/results/2026-04-28-mnq-comex-settle-pathway-b-v1-result.md
...
1468-1497). No session-start dependency, no break-bar dependency. break_dir
is used ONLY for direction segmentation of already-taken E2 trades (RULE 6.3
exception); it is NOT used as a predictor.

Reproduces independent SQL verified 2026-04-28: delta_IS=0.2286 exact match,
N_IS_on=199, ExpR_IS_on=+0.2453, ExpR_IS_off=+0.0166, Welch t=3.18, p=0.00161.
...
# spec (no later bar for EOD MTM). Verified live 2026-04-28: MNQ 99.72%
# scratch-pnl populated.

# e2-lookahead-policy: not-predictor
# garch_forecast_vol_pct is § 6.1 safe (prior-day-close input only; no
# break-bar dependency). orb_COMEX_SETTLE_break_dir is used ONLY for direction
# segmentation of already-taken trades (RULE 6.3 exception), NOT as a predictor.
...
PRE_REG_PATH = (
    ...
    / "audit"
    / "hypotheses"
    / "2026-04-28-mnq-comex-settle-garch-pathway-b-v1.yaml"
)
OUTPUT_PATH = (
    PROJECT_ROOT
    / "docs"
    / "audit"
    / "results"
    / "2026-04-28-mnq-comex-settle-pathway-b-v1-result.md"
)
...
K_FAMILY_VOLATILITY_PROVENANCE = 2700
...
def lane_correlation_with_deployed(con, on_trades: pd.DataFrame) -> float:
    """RULE 7 acknowledgement (pre-reg overlap concern: +0.773 vs deployed COMEX_SETTLE ORB_G5).

    Reproduces a Pearson correlation of per-trading_day pnl_r between this lane's
    'on' (signal=true) IS trades and the deployed COMEX_SETTLE ORB_G5 lane's IS trades.
    Descriptive only — does NOT gate the verdict.
    """
    sql = """
    SELECT o.trading_day, AVG(o.pnl_r) AS pnl_r
    FROM orb_outcomes o
    ...
    """
    deployed = con.execute(sql, [HOLDOUT_SACRED_FROM]).df()
    deployed["trading_day"] = pd.to_datetime(deployed["trading_day"])
    on_daily = on_trades.groupby("trading_day")["pnl_r"].mean().reset_index()
    merged = pd.merge(on_daily, deployed, on="trading_day", how="inner", suffixes=("_on", "_dep"))
    if len(merged) < 5:
        return float("nan")
    corr = merged["pnl_r_on"].corr(merged["pnl_r_dep"])
...
def emit_result(state: dict) -> None:
    ...
    s = state
    lines = []
    lines.append("# Phase D D4 — MNQ COMEX_SETTLE Pathway B K=1 — Result")
    lines.append("")
    lines.append("**Pre-reg:** docs/audit/hypotheses/2026-04-28-mnq-comex-settle-garch-pathway-b-v1.yaml")
    ...
    lines.append("")
    lines.append(
        "Pathway B K=1 confirmatory test of B-MNQ-COX (the highest-EV "
    ...
    lines.append("|---|---|---|---|")
    lines.append(f"| C5 DSR (Pathway A K_family={K_FAMILY_VOLATILITY_PROVENANCE}) [provenance] | ≥ {DSR_THRESHOLD} | {s['dsr_pa']:.4f} | {s['dsr_pa_verdict']} |")
    ...
    lines.append("")
    lines.append("## RULE 7 lane-correlation (descriptive — does not gate verdict)")
    lines.append("")
    lines.append(
        f"- Pearson correlation of daily pnl_r between this lane's IS_on trades and the "
        f"deployed `MNQ COMEX_SETTLE O5 RR1.5 ORB_G5` lane's IS trades: **{s['lane_corr']:.4f}**"
    )
    lines.append(
        "- Pre-reg flag: Phase B reported +0.773 vs deployed COMEX_SETTLE ORB_G5. "
        "RULE 7 threshold |corr| > 0.70 → portfolio overlap concern. "
        "Phase E admission requires an additivity audit before any capital allocation. "
        "This pre-reg's verdict is necessary, not sufficient, for live deployment."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append("```")
    lines.append(f"DUCKDB_PATH={GOLD_DB_PATH} python research/phase_d_d4_mnq_comex_settle_pathway_b.py")
    lines.append("```")
    lines.append("")
    ...
    lines.append("- DSR Pathway A K_family is approximated at 2700 (volatility family proxy from Phase B). Sensitivity")
    lines.append("  not exhaustive — true K may differ; the Pathway B K=1 path is what governs promotion here.")
    lines.append("- Lane-correlation +0.773 vs deployed COMEX_SETTLE ORB_G5 (RULE 7 overlap concern). A")
    lines.append("  CANDIDATE_READY or PARK verdict here does NOT clear Phase E portfolio admission.")
    lines.append("  Additivity audit (residualised vs deployed lane) required before any capital deployment.")
    lines.append("- This Pathway B test verifies the PHASE B EVIDENCE was correctly summarized; an honest")
    lines.append("  PARK or CANDIDATE_READY here does not assert independent edge over the deployed COMEX")
    lines.append("  ORB_G5 lane.")
    lines.append("")
    ...
    lines.append("- No portfolio additivity audit (RULE 7 overlap concern; deferred to Phase E)")
...
def run_pressure_test() -> str:
    """RULE 13: structural assertion that the test cannot silently swap in a banned feature."""
    safe_features = {
        "overnight_range_pct",
        "overnight_range",
        "atr_20",
    ...
    banned_predictors = {
        ...
        "mfe_r",
        "double_break",
        # E2 break-bar look-ahead bans (RULE 6.3)
...
def main():
    ...
    print("=== Phase D D4 — MNQ COMEX_SETTLE Pathway B K=1 ===")
    ...
    sr_var_per_trade_pa = 0.5 / max(is_years, 0.5)
    sr0_pa = sr0_threshold(sr_var_per_trade_pa, K_FAMILY_VOLATILITY_PROVENANCE)
    sr_var_per_trade_pb = 0.5 / max(is_years, 0.5)
    sr0_pb = sr0_threshold(sr_var_per_trade_pb, 1.0)
    ...
    if any_kill:
        ...
        if c6_verdict == "C6_PASS" and c8_verdict == "C8_PASS":
            ...
            verdict_explanation = (
                ...
                "RULE 7 portfolio additivity audit (lane-correlation +0.773 with "
                "deployed COMEX_SETTLE ORB_G5)."
            )
            ...
            verdict_explanation = (
                ...
                f"RULE 7 lane-correlation flag stands; Phase E admission requires "
                f"separate additivity audit."
            )
        else:
            final_verdict = "KILL"
            verdict_explanation = (
    ...
    # RULE 7 lane-correlation (descriptive)
    lane_corr = lane_correlation_with_deployed(con, is_on)
