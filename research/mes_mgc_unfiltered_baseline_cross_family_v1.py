"""MES + MGC unfiltered-baseline cross-family scan — Pathway A with BH-FDR.

Pre-reg: docs/audit/hypotheses/2026-04-20-mes-mgc-unfiltered-baseline-cross-family-v1.yaml

Cross-instrument extension of PR #51 (MNQ unfiltered-baseline cross-family).
Covers MES and MGC × (5m, 15m, 30m) × (RR=1.0, 1.5, 2.0) × full session universe
per instrument on unfiltered E2 CB1 baseline. Single BH-FDR applied to the
combined K_family (no post-hoc split).

K_family <= 180. BH-FDR at q=0.05. Chordia t >= +3.00 with theory.
...
PREREG_PATH = "docs/audit/hypotheses/2026-04-20-mes-mgc-unfiltered-baseline-cross-family-v1.yaml"
RESULT_DOC = Path("docs/audit/results/2026-04-20-mes-mgc-unfiltered-baseline-cross-family-v1.md")
INSTRUMENTS = ["MES", "MGC"]
APERTURES = [5, 15, 30]
RRS = [1.0, 1.5, 2.0]
MIN_N_IS = 100
...
def _list_sessions(con: duckdb.DuckDBPyConnection, instrument: str, orb_minutes: int) -> list[str]:
    rows = con.execute(
        """
...
def _test_cell(
    ...
    df = _load_cell(con, instrument, orb_minutes, rr, session)
...
def _render_table(cells: list[CellResult]) -> str:
    ...
    for c in sorted(
        ...
        key=lambda x: (
            ...
            x.aperture,
            x.rr,
            -x.net_expr_is if not np.isnan(x.net_expr_is) else 1e9,
        ),
    ):
        lines.append(
            f"| {c.instrument} | {c.aperture} | {c.rr} | {c.session} | {c.n_is} | "
...
def main() -> int:
    ...
    try:
        for inst in INSTRUMENTS:
            for apt in APERTURES:
                ...
                for rr in RRS:
                    for s in sessions:
                        r = _test_cell(con, inst, apt, rr, s)
                        if r is not None:
                            cells.append(r)
    finally:
        con.close()
    ...
    parts: list[str] = []
    parts.append("# MES + MGC unfiltered-baseline cross-family — v1\n")
    parts.append(f"**Pre-reg:** `{PREREG_PATH}`\n")
    parts.append(f"**Script:** `research/mes_mgc_unfiltered_baseline_cross_family_v1.py`\n")
    parts.append(f"**Family K:** {len(cells)} cells (N_IS >= {MIN_N_IS})\n")
    parts.append(
        "**Gates:** H1 (t>=+3.0, BH-FDR q<0.05), C6 (WFE>=0.50), "
        f"C8 (OOS/IS>=0.40, N_OOS>={MIN_N_OOS}), C9 (era stability)\n"
    )
    ...
    for inst in INSTRUMENTS:
        sub = [c for c in cells if c.instrument == inst]
        sub_cr = [c for c in sub if c.verdict == "CANDIDATE_READY"]
        sub_rs = [c for c in sub if c.verdict == "RESEARCH_SURVIVOR"]
        sub_kill = [c for c in sub if c.verdict == "KILL_IS"]
        parts.append(f"### {inst} breakdown")
        parts.append("")
        parts.append(f"- Cells tested: {len(sub)}")
        parts.append(f"- CANDIDATE_READY: {len(sub_cr)}")
        parts.append(f"- RESEARCH_SURVIVOR: {len(sub_rs)}")
        parts.append(f"- KILL_IS: {len(sub_kill)}")
        parts.append("")
    ...
    parts.append("")
    parts.append("## Comparison to MNQ PR #51")
    parts.append("")
    parts.append(
        "- PR #51 MNQ CANDIDATE_READYs: 5m RR=1.0 NYSE_OPEN; 5m RR=1.5 NYSE_OPEN; "
        "15m RR=1.0 NYSE_OPEN; 15m RR=1.0 US_DATA_1000; 15m RR=1.5 US_DATA_1000."
    )
    parts.append(
        "- Cross-instrument interpretation rule (pre-reg §decision_rules.family_summary):"
    )
    parts.append(
        "  - If MES/MGC CANDIDATE_READYs share MNQ sessions (NYSE_OPEN, US_DATA_1000): "
        "signal is cross-instrument, not MNQ-specific."
    )
    parts.append(
        "  - If MES/MGC CANDIDATE_READYs land on DIFFERENT sessions: new territory."
    )
    parts.append("  - If zero: MNQ signal does not generalise.")
    parts.append("")
    ...
    parts.append("- No monotonic-rank sizing overlay.")
    parts.append("- No filter-overlay testing (future pre-regs).")
    parts.append("- No re-test of MNQ (PR #51 is canonical for MNQ).")
    ...
    if cr:
        ...
        for c in sorted(cr, key=lambda x: -x.net_expr_is):
            print(
                f"  {c.instrument} {c.aperture}m RR={c.rr} {c.session}: "
