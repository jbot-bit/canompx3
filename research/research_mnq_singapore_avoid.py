#!/usr/bin/env python3
"""
research_mnq_singapore_avoid.py -- MNQ SINGAPORE_OPEN Compressed Spring AVOID
...
PRIOR FINDING:
    - Contracting ATR (atr_vel_ratio < 0.95) is the primary AVOID driver.
    - Compression tier barely matters within contracting regime.
    - Gap alignment = noise (p=0.62). Skipped here.
    - MNQ 1000 (old session name) E0/E1: both years negative (-0.404R, -0.448R)
...
CONFIGS TESTED:
    Primary: E2 CB1 RR3.0 O5 (dominant entry model for MNQ SINGAPORE_OPEN)
    Cross-checks: E1 CB1 RR2.5 O5, E2 CB1 RR3.0 O30
    Context: All MNQ sessions with E2 CB1 RR3.0 O5
...
def year_by_year(df: pd.DataFrame, label: str, vel_filter: str = "Contracting"):
    ...
    for yr in years:
        ...
        n_o_s = f"{n_o:>5}" if not np.isnan(n_o) else "  N/A"
...
PRIMARY_CONFIGS = [
    ...
    ("MNQ", "SINGAPORE_OPEN", 3.0, 1, "E2", 5,  "MNQ_SING_E2_CB1_RR3_O5 [PRIMARY]"),
    ("MNQ", "SINGAPORE_OPEN", 2.5, 1, "E1", 5,  "MNQ_SING_E1_CB1_RR2.5_O5 [XCHECK]"),
    ("MNQ", "SINGAPORE_OPEN", 3.0, 1, "E2", 30, "MNQ_SING_E2_CB1_RR3_O30 [XCHECK]"),
]

# Cross-session context: test other MNQ sessions at E2 CB1 RR3.0 O5
CONTEXT_SESSIONS = [
    "TOKYO_OPEN", "LONDON_METALS", "CME_REOPEN", "NYSE_OPEN",
    "US_DATA_830", "US_DATA_1000", "COMEX_SETTLE", "BRISBANE_1025",
]
...
def run():
    print()
    print("=" * 72)
    print("MNQ SINGAPORE_OPEN -- Compressed Spring AVOID Analysis")
    print("5-year dataset (2021-2025) | Event-based sessions")
    print("=" * 72)

    all_records = []
    yby_summaries = []

    # -- PART 1: SINGAPORE_OPEN primary + cross-checks -------------------------
    print("\n" + "=" * 72)
    print("PART 1: SINGAPORE_OPEN -- Primary + Cross-checks")
    print("=" * 72)
    ...
    # -- PART 2: Cross-session context (other MNQ sessions) --------------------
    print("\n\n" + "=" * 72)
    print("PART 2: CROSS-SESSION CONTEXT -- All MNQ sessions at E2 CB1 RR3.0 O5")
    print("=" * 72)

    for session in CONTEXT_SESSIONS:
        label = f"MNQ_{session}_E2_CB1_RR3_O5"
        print(f"\n{SEP}")
        print(f"  {label}")
        print(SEP)

        df = load("MNQ", session, 3.0, 1, "E2", 5)
        if df.empty or len(df) < 30:
            print(f"  SKIP -- insufficient data (N={len(df)})")
            continue
        ...
        # Quick: baseline + 9 combos only (no full layered analysis)
        ...
        # Contracting-only aggregate for this session
        con_mask = df["atr_vel_regime"] == "Contracting"
        con_r = df[con_mask]["pnl_r"]
        n_c, avg_c, wr_c, t_c, p_c = ttest_1s(con_r)
    ...
    for local_i, (orig_i, r) in enumerate(valid):
        ...
        if r["layer"] == "L0_baseline":
            continue  # baseline significance = config quality, not signal
    ...
    if avoid_survivors:
        ...
        for r in sorted(avoid_survivors, key=lambda x: x.get("avg_r", 0)):
            print(f"    [{r['config']:<40}]  {r['group']:<25}  "
    ...
    if other_survivors:
        ...
        for r in other_survivors:
            ...
            print(f"    [{r['config']:<40}]  {r['group']:<25}  p={r['p']:.4f}  avgR={avg_s}")

    # -- PART 4: Year-by-year summary -------------------------------------------
    print("\n\n" + "=" * 72)
    print("YEAR-BY-YEAR SUMMARY: Contracting ATR AVOID per session")
    print("=" * 72)
    print(f"  {'Config':<42} {'Neg':>4} {'Tot':>4} {'Hit%':>6}")
    for s in yby_summaries:
        print(f"  {s['config']:<42} {s['neg_years']:>4} {s['valid_years']:>4} "
              f"{s['pct_neg']:>6.0%}")

    # -- PART 5: Anomaly scan across sessions -----------------------------------
    print("\n\n" + "=" * 72)
    print("ANOMALY SCAN: vel x comp combos consistent across MNQ sessions")
    ...
    # Classification
    sing_avoids = [r for r in avoid_survivors if "SING" in r.get("config", "")]
    if sing_avoids:
        # Check year-by-year for SINGAPORE_OPEN configs
        sing_yby = [s for s in yby_summaries if "SING" in s["config"] and "PRIMARY" in s["config"]]
        if sing_yby:
            s = sing_yby[0]
            ...
            classification = "UNCLASSIFIED (no primary YBY data)"

        print(f"\n  SINGAPORE_OPEN AVOID classification: {classification}")
    else:
        print("\n  SINGAPORE_OPEN: No BH-sig AVOID. Signal is NOISE for this session.")
        classification = "NO SIGNAL"

    print()
    print("  MANDATORY DISCLOSURES:")
    print(f"    Instrument: MNQ (Micro Nasdaq)")
    print(f"    Session: SINGAPORE_OPEN (SGX/HKEX open 9:00 AM SGT)")
    print(f"    Period: all available MNQ data in gold.db (~5 years)")
    print(f"    Entry models tested: E1, E2")
    print(f"    IS/OOS: In-sample only -- NO holdout OOS performed")
    print(f"    BH FDR applied: YES (q=0.10, single correction across all tests)")
    print(f"    Total tests in FDR pool: {len(p_vals)}")
    print(f"    Mechanism: Contracting ATR = de-volatilizing market = breaks lack follow-through")
    print(f"    What kills it: MNQ-specific regime shift, SINGAPORE_OPEN structure change")
    ...
    df_out.to_csv(OUTPUT_DIR / "mnq_singapore_avoid_all.csv", index=False)
    ...
    if yby_rows:
        pd.DataFrame(yby_rows).to_csv(
            OUTPUT_DIR / "mnq_singapore_avoid_yearly.csv", index=False
        )

    print(f"\n  Output: {OUTPUT_DIR}/mnq_singapore_avoid_*.csv")
    print()

    return classification, avoid_survivors, boost_survivors
