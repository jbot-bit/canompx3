...
def run():
    ...
    for s in yby_summaries:
        print(f"  {s['config']:<42} {s['neg_years']:>4} {s['valid_years']:>4} {s['pct_neg']:>6.0%}")
    ...
    if yby_rows:
        pd.DataFrame(yby_rows).to_csv(OUTPUT_DIR / "mnq_singapore_avoid_yearly.csv", index=False)
