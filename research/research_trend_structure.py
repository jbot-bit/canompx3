...
for session in sessions:
    ...
    """
    rows = [dict(zip(
        ['sym','day','yr','struct','day_type','prev_dir','break_dir','outcome','mfe','mae'],
        r)) for r in con.execute(q).fetchall()]
    ...
    for sym in SYMBOLS:
        ...
        for struct_label, trend_dir, counter_dir in [
            ...
            if trend_dir:
                ...
                if n_a >= MIN_N:
                    print(f"    {struct_label:<22} aligned:  n={n_a:3d}  WR={wr_a:.1%}  "
                          f"uplift={uplift:+.1%}  yrs={p_a}/{t_a}{flag}")
                if n_c >= MIN_N:
                    up_c = wr_c - baseline_wr
                    print(f"    {struct_label:<22} counter:  n={n_c:3d}  WR={wr_c:.1%}  "
                          f"uplift={up_c:+.1%}")
            else:
                ...
                if n_s >= MIN_N:
                    print(f"    {struct_label:<22} all:      n={n_s:3d}  WR={wr_s:.1%}  "
        ...
        if len(dt_rows) > MIN_N:
            ...
            for dt, aligned_dir in [('TREND_UP', 'long'), ('TREND_DOWN', 'short'),
                                     ('BALANCED', None), ('NON_TREND', None)]:
                ...
                if aligned_dir:
                    ...
                    flag = " << SIGNAL" if uplift > 0.04 and p_a >= max(t_a * 0.6, 2) else ""
                    if n_a >= MIN_N:
                        print(f"    {dt:<15} aligned: n={n_a:3d}  WR={wr_a:.1%}  "
                              f"uplift={uplift:+.1%}  yrs={p_a}/{t_a}{flag}")
                else:
                    ...
                    uplift = wr_s - baseline_wr
                    if n_s >= MIN_N:
                        print(f"    {dt:<15} all:     n={n_s:3d}  WR={wr_s:.1%}  "
                              f"uplift={uplift:+.1%}  yrs={p_s}/{t_s}")
