...
def main() -> int:
    ...
    parts.append("")
    parts.append(
        "`CONTINUE` as role-aware evidence. "
        "The implementation study supports treating PR48 as a conditional-role question first, "
        "not forcing it into standalone-or-dead framing."
    )
    parts.append("")
    ...
    parts.append("")
    parts.append("- This study is descriptive role evidence, not a promotion gate by itself.")
    parts.append("- The frozen quintile mapping is IS-defined and OOS-monitored only.")
    parts.append("- Current repo runtime surfaces do not yet consume these role outputs natively.")
    parts.append("")
