...
def _run_ovnrng50_analysis() -> None:
    """OVNRNG_50_FAST10 PR #44 DSR correction analysis (original script body).

    Extracted from module top-level 2026-05-12 so importers (e.g.
    `research/sr_alarm_steps_3_4_5_2026_05_12.py`) can use `bailey_dsr` and
    `bailey_expected_max` as canonical helpers without triggering this
    100-line analysis on import. No behavioural change; output is identical
    when run as `python research/audit_ovnrng50_canonical_dsr.py`.
    """
