...
def test_threshold(df: pd.DataFrame, threshold: int) -> dict:
    ...
    if len(on) < 10 or len(off) < 10:
        return {
            ...
            "skip_reason": "thin bucket", "skip": True,
        }
    ...
    return {
        ...
        "p_boot": float(p_boot),
        "yr_lifts": yr_lifts,
        "pos_yrs": pos_yrs, "total_yrs": len(yr_lifts),
        "skip": False,
    }
...
def main() -> None:
        ...
        for direction in ["long", "short"]:
            ...
            if len(df) < 30:
                ...
                for th in GARCH_GRID:
                    results.append({
                        "lane": lane.name, "dir": direction, "threshold": th,
                        "skip": True, "reason": "IS N < 30",
                    })
                continue
            ...
            if nt:
                ...
                for b in nt:
                    print(f"    bucket {b['bucket']} [{b['garch_min']:.0f}-{b['garch_max']:.0f}] "
                          f"N={b['n']} ExpR={b['expr']:+.3f} WR={b['wr']:.1%}")

            # Threshold tests
            for th in GARCH_GRID:
                r = test_threshold(df, th)
                r.update({"lane": lane.name, "dir": direction, "threshold": th, "ntile": nt,
                          "n_oos": len(df_oos)})
                ...
                if not r.get("skip"):
                    print(f"  @ {th}: N_on={r['n_on']}/{r['n_off']}  "
    ...
    for s in sorted(survivors, key=lambda x: x["p_two"]):
        print(f"    {s['lane']} {s['dir']} @{s['threshold']}: "
...
def emit(lanes: list[Lane], results: list[dict], K: int) -> None:
    ...
    for lane in lanes:
        for direction in ["long", "short"]:
            ...
            if nt:
                lines += ["NTILE-5 by garch_pct (informational — monotonicity check):", "",
                          "| bucket | garch range | N | ExpR | WR |",
                          "|---|---|---|---|---|"]
                for b in nt:
                    lines.append(f"| {b['bucket']} | {b['garch_min']:.0f}-{b['garch_max']:.0f} | "
                                 f"{b['n']} | {b['expr']:+.3f} | {b['wr']:.1%} |")
                lines.append("")

            lines += ["Threshold tests:", "",
    ...
    lines += ["---", "", "## Summary — BH-FDR at K=" + str(K), "",
    ...
    if survivors:
        ...
        for s in sorted(survivors, key=lambda x: x["p_two"]):
            ...
            lines.append(f"| {s['lane']} | {s['dir']} | {s['threshold']} | {s['lift']:+.3f} | "
                         f"{s['p_two']:.5f} | {s['pos_yrs']}/{s['total_yrs']} | {oos} |")
    lines.append("")

    # Deployment-role recommendation
    lines += ["---", "", "## Interpretation & deployment roles", "",
