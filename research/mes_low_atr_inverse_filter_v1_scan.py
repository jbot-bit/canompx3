#!/usr/bin/env python3
"""MES low-ATR (~ATR_P70) inverse-filter Pathway A family — K=20.

Pre-registered at:
  docs/audit/hypotheses/2026-04-19-mes-low-atr-inverse-filter-v1.yaml
  LOCKED at commit b9f38e33.

5 sessions × 2 directions × RR=1.5 × (~ATR_P70) = 20 cells.

The filter mask is computed as the LOGICAL INVERSION of the canonical
ATR_P70 mask — `mask = ~filter_signal(df, 'ATR_P70', sess)`. This stays
canonical-delegated (no inline re-implementation of the filter logic)
while testing the hypothesis that bottom-70% ATR MES days are the
profitable regime (inverse of the K=40 parent's 10/10 negative ATR_P70
observation).
...
RESULT_PATH = PROJECT_ROOT / "docs/audit/results/2026-04-19-mes-low-atr-inverse-filter-v1-scan.md"
PREREG_PATH = "docs/audit/hypotheses/2026-04-19-mes-low-atr-inverse-filter-v1.yaml"
INSTRUMENT = "MES"
ORB_MINUTES = 5
RR = 1.5

# Scope lock per pre-reg b9f38e33
