...
def run_analysis(db_path=None):
    ...
    for min_size, flabel in [(6.0, "G6+"), (8.0, "G8+")]:
        for rr in [2.0, 2.5]:
            for cb in [4]:
                subset = known[
                    (known["rr_target"] == rr) & (known["confirm_bars"] == cb) & (known["orb_size_1100"] >= min_size)
                ]
                if len(subset) < 15:
                    continue
                a = subset[subset["ib_aligned"] == True]
    ...
    print(
        f"  {'Year':<6} {'N_aln':>6} {'WR_aln':>7} {'ExpR_aln':>9} {'N_opp':>6} {'WR_opp':>7} {'ExpR_opp':>9} {'Delta':>7}"
    )
    print("  " + "-" * 65)
    for year in sorted(yearly_sub["year"].unique()):
        ...
        print(
            f"  {year:<6} {len(a_pnl):>6} {a_wr:>6.1%} {a_expr:>+9.3f} "
