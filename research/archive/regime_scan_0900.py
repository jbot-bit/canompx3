...
for i, r in enumerate(results[:15]):
    print(
        f"  {i + 1:>2}. {r['config']:<30} N={r['n']:<4} ExpR={r['expr']:>+.4f}  TotalR={r['totr']:>+.1f}  WR={r['wr']:.0f}%"
    )
...
for dow_name, dow_val in [("Mon", 1), ("Tue", 2), ("Wed", 3), ("Thu", 4), ("Fri", 5)]:
    on = get_pnl("0900", "E1", 2.5, 2, 4.0, f"AND EXTRACT(isodow FROM o.trading_day) = {dow_val}", regime=True)
    off = get_pnl("0900", "E1", 2.5, 2, 4.0, f"AND EXTRACT(isodow FROM o.trading_day) != {dow_val}", regime=True)
    ...
    test_results.append(
        {
            ...
            "n_on": len(dst_on),
            "mean_on": np.mean(dst_on),
            "n_off": len(dst_off),
            "mean_off": np.mean(dst_off),
...
g4to6 = get_pnl("0900", "E1", 2.5, 2, 4.0, "AND d.orb_0900_size < 6", regime=True)
    ...
    test_results.append(
        {
            "name": "G6+_vs_G4-6",
            "n_on": len(g6plus),
            "mean_on": np.mean(g6plus),
            "n_off": len(g4to6),
            "mean_off": np.mean(g4to6),
...
for i, r in enumerate(test_results):
    ...
    print(
        f"  {r['name']:<25} ON: N={r['n_on']:<3} ExpR={r['mean_on']:>+.4f}  |  OFF: N={r['n_off']:<3} ExpR={r['mean_off']:>+.4f}  |  delta={r['delta']:>+.4f}  p={r['p']:.4f} {raw} {sig}"
    )
