...
for level_name, long_level_col, short_level_col in ext_levels:
    ...
    for sym in SYMBOLS:
        ...
        for dir_, res in results.items():
            ...
            base_rev = [r for r in res if not r["tagged"] and to_r((r.get("orb_h", 0) or 0) - 0, 1) is not None]
            # simpler: just check baseline outcome
            base_close_below = sum(
                1 for r in non_tagged if r.get("revert_r") is not None and r["revert_r"] and r["revert_r"] > 0
            )
