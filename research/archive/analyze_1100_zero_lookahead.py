...
def run_analysis(db_path=None):
    ...
    for rr in [2.0, 2.5]:
        ...
        print(f"\n  E1 CB4 RR{rr} G4+:")
        for label, group in [
            ("Gap Aligned", subset[subset["gap_aligned"]]),
    ...
    for rr in [2.0, 2.5]:
        ...
        print(f"\n  E1 CB4 RR{rr} G4+:")
        for label, group in [
            ("Trend Aligned", subset[subset["trend_aligned"]]),
    ...
    for rr in [2.0, 2.5]:
        ...
        print(f"\n  E1 CB4 RR{rr} G4+:")
        for label, group in [
            ("High Vol", subset[subset["high_vol"]]),
    ...
    for rr in [2.0, 2.5]:
        ...
        print(f"\n  E1 CB4 RR{rr} G4+:")
        for label, group in [
            ("Big 0900", subset[subset["big_0900"]]),
    ...
    for year in sorted(yearly_data["year"].unique()):
        ...
        print(
            f"  {year:<6} {aln_n:>6} {aln_wr:>6.1%} {aln_expr:>+9.3f} {opp_n:>6} {opp_wr:>6.1%} {opp_expr:>+9.3f} {delta:>+7.3f}"
        )
...
def _print_stats(label, group):
    ...
    dd = (cum - peak).min()
    print(
        f"    {label:<20} N={n:>4}  WR={wr:>5.1%}  ExpR={expr:>+.3f}  "
