...
def _session_start_utc(session: str, trading_day) -> datetime | None:
    ...
    local_dt = datetime.combine(td, datetime.min.time().replace(hour=hour, minute=minute), tzinfo=_BRISBANE)
    return local_dt.astimezone(timezone.utc)


def compute_volume_ratio_for_day(
    con, instrument: str, trading_day, session: str, entry_ts, orb_minutes: int = 5
) -> float | None:
    ...
    orb_bars = con.execute(
        ...
        ORDER BY ts_utc
    """,
        [instrument, session_start_utc, orb_end_utc],
    ).fetchall()
    ...
    if break_bar is None or break_bar[0] is None or break_bar[0] == 0:
        ...
        break_bar = con.execute(
            ...
            LIMIT 1
        """,
            [instrument, entry_ts, entry_ts, entry_ts],
        ).fetchone()
...
def main():
    ...
    parser.add_argument(
        "--sessions", nargs="*", default=None, help="Specific sessions to analyze (default: all enabled)"
    )
    parser.add_argument("--sample-days", type=int, default=0, help="Sample N random days per session (0 = all)")
    ...
    for session in focus_sessions:
        ...
        for em, cb in [("E1", 2), ("E3", 1)]:
            em_data = outcomes[(outcomes["entry_model"] == em) & (outcomes["confirm_bars"] == cb)]
            if len(em_data) < 20:
                continue
            ...
            for rr in [1.5, 2.0, 2.5, 3.0]:
                ...
                ratios = []
                print(f"\n    RR{rr}: computing volume ratios for {len(rr_data)} trades...", end="", flush=True)

                for _, row in rr_data.iterrows():
                    ratio = compute_volume_ratio_for_day(con, instrument, row["trading_day"], session, row["entry_ts"])
                    ratios.append(ratio)
                ...
                print(
                    f"    Volume ratio stats: mean={vr.mean():.2f}, "
                ...
                normal = has_ratio[(has_ratio["vol_ratio"] >= 0.8) & (has_ratio["vol_ratio"] < 1.5)]
                ...
                if "mfe_r" in has_ratio.columns and has_ratio["mfe_r"].notna().sum() > 10:
                    for label, sub in [("Surge", surge), ("Normal", normal), ("Weak", weak)]:
                        ...
                        if len(mfe_vals) >= 5:
                            print(f"    {label} MFE: mean={mfe_vals.mean():.2f}R, median={mfe_vals.median():.2f}R")
