...
for sym in ["MES", "MNQ"]:
    ...
    for direction in ["LONG", "SHORT"]:
        ...
        if len(grp) >= 10:
            ...
            avg = np.nanmean(grp)
            print(
                f"  {sym} 1000 when MGC 0900={direction}: N={len(grp)}, avgR={avg:+.3f}, "
                f"baseline={np.nanmean(baseline):+.3f}, p={fmt_pval(p)}, d={d:+.3f}, "
                f"{sample_label(len(grp))}"
            )
            q1_results.append({"label": f"{sym}_1000_mgc0900_{direction}", "N": len(grp), "avg_r": avg, "p": p, "d": d})
    ...
    for align_type in ["aligned", "opposed"]:
        ...
        if len(grp) >= 10 and len(other) >= 10:
            ...
            avg = np.nanmean(grp)
            print(
                f"  {sym} 1000 {align_type} with MGC 0900: N={len(grp)}, avgR={avg:+.3f}, "
                f"vs opposite={np.nanmean(other):+.3f}, p={fmt_pval(p)}, d={d:+.3f}, "
                f"{sample_label(len(grp))}"
            )
            q1_results.append(
                {"label": f"{sym}_1000_mgc_align_{align_type}", "N": len(grp), "avg_r": avg, "p": p, "d": d}
            )
...
all_wide = (
    (df_pivot["mgc_size"] > df_pivot["mgc_size_med"])
...
for sym in ["MGC", "MES", "MNQ"]:
    ...
    for conc in ["all_wide", "all_narrow", "mixed"]:
        ...
        if len(grp) >= 10:
            ...
            avg = np.nanmean(grp)
            print(
                f"  {sym} 1000 {conc}: N={len(grp)}, avgR={avg:+.3f}, "
                f"baseline={np.nanmean(other):+.3f}, p={fmt_pval(p)}, d={d:+.3f}, "
                f"{sample_label(len(grp))}"
            )
            q2_results.append({"label": f"{sym}_1000_{conc}", "N": len(grp), "avg_r": avg, "p": p, "d": d})
...
for sym in ["MGC", "MES", "MNQ"]:
    ...
    if len(aligned) >= 10 and len(opposed) >= 10:
        ...
        print(
            f"  {sym} 1000 gap_aligned: N={len(aligned)}, avgR={np.nanmean(aligned):+.3f}, {sample_label(len(aligned))}"
        )
        print(
            f"  {sym} 1000 gap_opposed: N={len(opposed)}, avgR={np.nanmean(opposed):+.3f}, {sample_label(len(opposed))}"
        )
        print(f"    delta={np.nanmean(aligned) - np.nanmean(opposed):+.3f}, p={fmt_pval(p)}, d={d:+.3f}")
        q3_results.append(
            {"label": f"{sym}_1000_gap_aligned", "N": len(aligned), "avg_r": np.nanmean(aligned), "p": p, "d": d}
        )
        q3_results.append(
            {"label": f"{sym}_1000_gap_opposed", "N": len(opposed), "avg_r": np.nanmean(opposed), "p": p, "d": d}
        )
...
for sym in ["MGC", "MES", "MNQ"]:
    ...
    if len(big_gap) >= 10 and len(small_gap) >= 10:
        ...
        print(
            f"  {sym} 1000 big_gap(>{med_gap:.1f}pts): N={len(big_gap)}, avgR={np.nanmean(big_gap):+.3f}, {sample_label(len(big_gap))} "
            f"vs small: N={len(small_gap)}, avgR={np.nanmean(small_gap):+.3f}, p={fmt_pval(p)}"
        )
        q3_results.append(
            {"label": f"{sym}_1000_big_gap_vs_small", "N": len(big_gap), "avg_r": np.nanmean(big_gap), "p": p, "d": d}
        )
...
for sym in ["MGC", "MES", "MNQ"]:
    ...
    if len(brk) >= 10 and len(no_brk) >= 10:
        ...
        print(
            f"  {sym}: 0900 break exists -> 1000: N={len(brk)}, avgR={np.nanmean(brk):+.3f}, {sample_label(len(brk))}"
        )
        print(
            f"  {sym}: 0900 no break   -> 1000: N={len(no_brk)}, avgR={np.nanmean(no_brk):+.3f}, {sample_label(len(no_brk))}"
        )
        print(f"    delta={np.nanmean(brk) - np.nanmean(no_brk):+.3f}, p={fmt_pval(p)}")
        q4_results.append({"label": f"{sym}_0900brk_to_1000", "N": len(brk), "avg_r": np.nanmean(brk), "p": p, "d": d})
    ...
    if len(sub_with_dir) >= 20:
        ...
        for cas_type in ["same", "flip"]:
            ...
            if len(grp) >= 10 and len(other) >= 10:
                t, p, d = welch_t(grp, other)
                print(
                    f"  {sym} 0900->1000 {cas_type}: N={len(grp)}, avgR={np.nanmean(grp):+.3f}, p={fmt_pval(p)}, {sample_label(len(grp))}"
                )
                q4_results.append(
                    {"label": f"{sym}_cascade_{cas_type}", "N": len(grp), "avg_r": np.nanmean(grp), "p": p, "d": d}
                )
...
for vol in ["high_vol", "low_vol"]:
    ...
    if len(fri) >= 10 and len(not_fri) >= 10:
        ...
        q5_results.append({"label": f"MES_1000_fri_{vol}", "N": len(fri), "avg_r": np.nanmean(fri), "p": p, "d": d})
...
for regime in ["expanding", "contracting"]:
    ...
    if len(grp) >= 10:
        t, p, d = welch_t(grp, other)
        print(
            f"  Friday {regime}: N={len(grp)}, avgR={np.nanmean(grp):+.3f}, p={fmt_pval(p)}, {sample_label(len(grp))}"
        )
        q5_results.append({"label": f"MES_1000_fri_{regime}", "N": len(grp), "avg_r": np.nanmean(grp), "p": p, "d": d})
...
for sym in ["MGC", "MNQ"]:
    ...
    if len(df_5b) >= 30:
        ...
        for vol_label, mask in [
            ("high_vol", df_5b["atr_20"] > df_5b["exp_med"]),
            ...
            if len(fri) >= 10 and len(not_fri) >= 10:
                t, p, d = welch_t(fri, not_fri)
                print(
                    f"  {sym} {vol_label} Friday: N={len(fri)}, avgR={np.nanmean(fri):+.3f}, {sample_label(len(fri))} "
                    f"vs Mon-Thu: avgR={np.nanmean(not_fri):+.3f}, p={fmt_pval(p)}"
                )
                q5_results.append(
                    {"label": f"{sym}_1000_fri_{vol_label}", "N": len(fri), "avg_r": np.nanmean(fri), "p": p, "d": d}
                )
...
with open(OUTPUT, "w") as f:
    ...
    for section, results in findings:
        ...
        for r in results:
            ...
            f.write(
                f"- {r['label']}: N={r['N']}, avgR={r['avg_r']:+.3f}, "
    ...
    f.write(
        "- Sample labels: INVALID(<30) / REGIME(30-99) / PRELIMINARY(100-199) / CORE(200-499) / HIGH-CONFIDENCE(500+)\n"
    )
    f.write("\n## Caveats\n")
