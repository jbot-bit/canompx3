...
def sim_fixed_target(ts, highs, lows, closes, entry_idx, entry_price, stop_price, target_price, is_long, cutoff_ts):
    ...
    for i in range(entry_idx, len(ts)):
        ...
        if is_long and lo <= stop_price:
            return to_r_multiple(SPEC, entry_price, stop_price, stop_price - entry_price), "stop"
        if not is_long and h >= stop_price:
            return to_r_multiple(SPEC, entry_price, stop_price, entry_price - stop_price), "stop"
        # Target
        if is_long and h >= target_price:
            return to_r_multiple(SPEC, entry_price, stop_price, target_price - entry_price), "target"
        if not is_long and lo <= target_price:
            return to_r_multiple(SPEC, entry_price, stop_price, entry_price - target_price), "target"
        # Time
        if ts[i] >= cutoff_ts:
...
def sim_exploit(
    ...
    for i in range(entry_idx, len(ts)):
        ...
        if is_long and lo <= stop_price:
            ...
            return (to_r_multiple(SPEC, entry_price, stop_price, stop_price - entry_price), "stop", phase)
        if not is_long and h >= stop_price:
            phase = "limbo" if not alignment_known else alignment
            return (to_r_multiple(SPEC, entry_price, stop_price, entry_price - stop_price), "stop", phase)
        ...
        if not alignment_known:
            ...
            if is_long and h >= target_price:
                return (
                    to_r_multiple(SPEC, entry_price, stop_price, target_price - entry_price),
                    "limbo_target",
                    "limbo",
                )
            if not is_long and lo <= target_price:
                return (
                    to_r_multiple(SPEC, entry_price, stop_price, entry_price - target_price),
                    "limbo_target",
                    "limbo",
                )
            ...
            if ib_break_idx is not None and i >= ib_break_idx:
                ...
                if alignment == "opposed":
                    pnl = (c - entry_price) if is_long else (entry_price - c)
                    return (to_r_multiple(SPEC, entry_price, stop_price, pnl), "opposed_kill", "opposed")
            ...
            if ts[i] >= cutoff_ts:
                pnl = (c - entry_price) if is_long else (entry_price - c)
                return (to_r_multiple(SPEC, entry_price, stop_price, pnl), "limbo_time", "limbo")
            continue
        ...
        if alignment == "aligned":
            ...
            if ts[i] >= cutoff_ts:
                pnl = (c - entry_price) if is_long else (entry_price - c)
                return (to_r_multiple(SPEC, entry_price, stop_price, pnl), "time_7h", "aligned")
            continue
        ...
        if alignment == "no_break":
            ...
            if ts[i] >= cutoff_ts:
                pnl = (c - entry_price) if is_long else (entry_price - c)
                return (to_r_multiple(SPEC, entry_price, stop_price, pnl), "nobreak_time", "no_break")
            continue
...
def sim_pyramid(
    ...
    for i in range(entry_idx, len(ts)):
        ...
        if is_long and lo <= current_stop:
            pnl1 = to_r_multiple(SPEC, entry_price, stop_price, current_stop - entry_price)
            pnl2 = 0.0
            if pyramided:
        ...
        if not is_long and h >= current_stop:
            pnl1 = to_r_multiple(SPEC, entry_price, stop_price, entry_price - current_stop)
            pnl2 = 0.0
            if pyramided:
        ...
        if not alignment_known:
            ...
            if ib_break_idx is not None and i >= ib_break_idx:
                ...
                if ib_break_dir is None:
                    ...
                    alignment = "opposed"
                    pnl = (c - entry_price) if is_long else (entry_price - c)
                    return (to_r_multiple(SPEC, entry_price, stop_price, pnl), "opposed_kill", "opposed", 1)
                continue

            if ts[i] >= cutoff_ts:
                pnl = (c - entry_price) if is_long else (entry_price - c)
                return (to_r_multiple(SPEC, entry_price, stop_price, pnl), "limbo_time", "limbo", 1)
            continue
        ...
        if alignment == "no_break":
            ...
            if ts[i] >= cutoff_ts:
                pnl = (c - entry_price) if is_long else (entry_price - c)
                return (to_r_multiple(SPEC, entry_price, stop_price, pnl), "nobreak_time", "no_break", 1)
            continue
...
def print_report(pdf, session_label):
    ...
    # --- Main comparison ---
    print(f"\n  {'Strategy':22s} {'N':>5s} {'WR':>7s} {'ExpR':>8s} {'Sharpe':>8s} {'MaxDD':>8s} {'Total':>8s}")
    ...
    print(f"  {'Year':5s} {'N':>4s} {'Aln':>4s} {'Opp':>4s} {'Fixed':>8s} {'Exploit':>8s} {'Pyramid':>8s} {'Best':>8s}")
    print(f"  {'-' * 5} {'-' * 4} {'-' * 4} {'-' * 4} {'-' * 8} {'-' * 8} {'-' * 8} {'-' * 8}")
    ...
    for year in sorted(pdf["year"].unique()):
        ...
        if mf and me and mp:
            sharpes = {"fixed": mf["sharpe"], "exploit": me["sharpe"], "pyramid": mp["sharpe"]}
            best = max(sharpes, key=sharpes.get)
            print(
                f"  {year:5s} {len(ydf):>4d} {na:>4d} {no:>4d} "
    ...
    if mf and me and mp:
        print(
            f"  {'TOTAL':5s} {n:>4d} {len(aligned):>4d} {len(opposed):>4d} "
    ...
    if len(pyr_aligned) > 0:
        ...
        if m:
            print(f"    WR={m['wr']:.1%}  ExpR={m['expr']:+.3f}R  Total={m['total']:+.1f}R  MaxDD={m['maxdd']:.2f}R")
...
def run_checks(pdf, session_label):
    ...
    if len(opposed) > 0:
        ...
        limbo_or_kill = opposed["exploit_reason"].isin({"opposed_kill", "limbo_target", "limbo_time", "stop"}).all()
        if limbo_or_kill:
            print(f"    [PASS] opposed trades: all exit via kill or limbo ({len(opposed)} trades)")
            ok += 1
        else:
    ...
    if len(aligned) >= 10:
        ...
        if mc and me:
            ...
            print(
                f"    [INFO] aligned: ctrl ExpR={mc['expr']:+.3f} "
