...
"""
rows = [
    dict(zip(["sym", "yr", "open_ctx", "london_ctx", "on_pos", "break_dir", "outcome", "mfe"], r))
...
for sym in SYMBOLS:
    ...
    for ctx in ["gap_up_above_pdh", "gap_up", "gap_down", "gap_down_below_pdl", "flat"]:
        ...
        for r in aligned:
            by_yr[r["yr"]].append(1 if r["outcome"] == "win" else 0)
        p, t = yr_consist(by_yr)
        uplift = wr_a - base_wr
        flag = " << SIGNAL" if abs(uplift) > 0.05 and p >= max(t * 0.6, 2) else ""
        print(f"    {ctx:<30} aligned n={len(aligned):3d}  WR={wr_a:.1%}  uplift={uplift:+.1%}  yrs={p}/{t}{flag}")
    ...
    for pos, aligned_dir in [("open_top_half", "long"), ("open_bottom_half", "short")]:
        ...
        uplift = wr_a - base_wr
        flag = " << SIGNAL" if abs(uplift) > 0.05 and p >= max(t * 0.6, 2) else ""
        print(f"    {pos:<30} aligned n={len(aligned):3d}  WR={wr_a:.1%}  uplift={uplift:+.1%}  yrs={p}/{t}{flag}")
...
"""
rows2 = [
    dict(zip(["sym", "yr", "struct", "pdh_hit", "pdl_hit", "break_dir", "outcome", "mfe"], r))
...
for sym in SYMBOLS:
    ...
    for struct, aligned_dir, counter_dir in [
        ...
        if aligned_dir:
            ...
            print(
                f"    {struct:<15} aligned n={len(aligned):3d}  WR={wr_a:.1%}  uplift={uplift:+.1%}  yrs={p}/{t}{flag}"
            )
        else:
            if len(grp) < MIN_N:
                continue
            ...
            uplift = wr_g - base_wr
            flag = " << SIGNAL" if abs(uplift) > 0.04 and p >= max(t * 0.6, 2) else ""
            print(f"    {struct:<15} all    n={len(grp):3d}  WR={wr_g:.1%}  uplift={uplift:+.1%}  yrs={p}/{t}{flag}")
    ...
    for label, grp in [("pdh_hit→short", pdh_short), ("pdl_hit→long", pdl_long)]:
        ...
        uplift = wr_g - base_wr
        flag = " << SIGNAL" if abs(uplift) > 0.04 and p >= max(t * 0.6, 2) else ""
        print(f"    {label:<20} n={len(grp):3d}  WR={wr_g:.1%}  uplift={uplift:+.1%}  yrs={p}/{t}{flag}")
