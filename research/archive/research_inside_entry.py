...
def load_orb_days(con) -> pd.DataFrame:
    ...
    """,
        [INSTRUMENT, INSTRUMENT, ORB_MIN_PTS],
    ).df()
...
def scan_outcome(
    bars: pd.DataFrame, fill_bar_i: int, fill_dir: str, fill_price: float, stop: float, target: float
) -> dict:
...
def simulate_e0_baseline(
    bars: pd.DataFrame, orb_high: float, orb_low: float, orb_size: float, rr: float
) -> dict | None:
    """
...
def simulate_inside_entry(
    bars: pd.DataFrame, orb_high: float, orb_low: float, orb_size: float, offset: float, rr: float
) -> dict | None:
    """
...
def run_all_days(days_df, bars_by_day, offset, rr, use_e0_baseline=False):
    ...
    for _, day in days_df.iterrows():
        ...
        if use_e0_baseline:
            result = simulate_e0_baseline(day_bars, day["orb_high"], day["orb_low"], day["orb_size"], rr)
        else:
            result = simulate_inside_entry(day_bars, day["orb_high"], day["orb_low"], day["orb_size"], offset, rr)
        if result is None:
            continue
...
def main():
    ...
    for label, group in [("ALL days", all20), ("prev=LOSS days", pl20), ("prev=WIN days", pw20)]:
        ...
        print(
            f"    Short fills: {len(shorts):3d}  ({1 - pct_l:.1%})  avgR={np.mean([r['pnl_r'] for r in shorts]):+.3f}"
        )
        print(
            f"    Direction bias test: p={p:.4f}  {'(BIASED - one side is more likely)' if p < 0.10 else '(NO BIAS - both sides equally likely)'}"
        )
