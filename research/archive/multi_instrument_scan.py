        ...
        for em in ["E1", "E3"]:
            for rr in [2.0, 2.5, 3.0]:
                ...
                for earlier_sess in earlier:
                    ...
                    try:
                        true_arr = get_pnl(con, instrument, sess, em, rr, cb, size_min, f"AND ({col}) = true")
                        false_arr = get_pnl(con, instrument, sess, em, rr, cb, size_min, f"AND ({col}) = false")
                    except Exception:
                        continue
                ...
                if dst_col:
                    try:
                        dst_on = get_pnl(con, instrument, sess, em, rr, cb, size_min, f"AND d.{dst_col} = true")
                        dst_off = get_pnl(con, instrument, sess, em, rr, cb, size_min, f"AND d.{dst_col} = false")
                            ...
                            results.append(
                                {
                                    "name": f"DST_{dst_col}",
                                    "n_true": len(dst_on),
                                    "mean_true": np.mean(dst_on),
                                    "n_false": len(dst_off),
                                    "mean_false": np.mean(dst_off),
                ...
                for earlier_sess in earlier[:2]:
                    ...
                    try:
                        small = get_pnl(
                            con, instrument, sess, em, rr, cb, size_min, f"AND d.{ecol} < 4 AND d.{ecol} IS NOT NULL"
                        )
                        big = get_pnl(
                            con, instrument, sess, em, rr, cb, size_min, f"AND d.{ecol} >= 4 AND d.{ecol} IS NOT NULL"
                        )
                ...
                for r in results:
                    ...
                    print(
                        f"  {r['name']:<30} delta={r['delta']:>+.4f}  p={r['p']:.4f} {raw:<4} N={r['n_true']}+{r['n_false']}  {sig}"
                    )
...
def run_regime_scan(con, instrument, sessions, size_min=4.0):
    ...
    for i, r in enumerate(results[:10]):
        print(
            f"  {i + 1:>2}. {r['config']:<35} N={r['n']:<4} ExpR={r['expr']:>+.4f}  TotalR={r['totr']:>+.1f}  WR={r['wr']:.0f}%"
        )
    ...
    for r in results[:5]:
        ...
        """).fetchone()
        print(
            f"  {r['config']:<35} 2025+: N={r['n']:<4} ExpR={r['expr']:>+.4f} | ALL: N={full[0]:<5} ExpR={full[1]:>+.4f}"
        )
...
def main():
    ...
    parser.add_argument(
        "--instrument", type=str, default=None, help="Single instrument (MES, MNQ, MGC). Default: all three."
    )
    parser.add_argument("--size-min", type=float, default=4.0, help="Minimum ORB size filter (default: 4.0)")
    parser.add_argument("--edge-only", action="store_true", help="Only run edge hunter (skip regime scan)")
    parser.add_argument("--regime-only", action="store_true", help="Only run regime scan (skip edge hunter)")
