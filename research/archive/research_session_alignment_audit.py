...
def main():
    ...
    print("=" * 90)
    print(
        f"Baseline: entry_model={ENTRY_MODEL}, rr_target={RR_TARGET}, "
        ...
        for instrument in ACTIVE_INSTRUMENTS:
            ...
            for season_label, is_summer_val in [("SUMMER", True), ("WINTER", False)]:
                ...
                if fixed_stats["n"] >= MIN_TRADES and dynamic_stats["n"] >= MIN_TRADES:
                    t_stat, p_val = stats.ttest_ind(fixed_pnl.values, dynamic_pnl.values, equal_var=False)

                delta = (
                    dynamic_stats["avg_r"] - fixed_stats["avg_r"]
                    if not (np.isnan(dynamic_stats["avg_r"]) or np.isnan(fixed_stats["avg_r"]))
                    else np.nan
                )
    ...
    for _, row in rdf.iterrows():
        ...
        print(
            f"{pair:<22} {row['instrument']:<5} {row['season']:<7} | "
    ...
    summer_valid = summer[(summer["fixed_n"] >= MIN_TRADES) & (summer["dynamic_n"] >= MIN_TRADES)]
    ...
    if summer_valid.empty:
        ...
        print(
            f"Dynamic outperforms fixed: {len(dynamic_better)}/{len(summer_valid)} "
        ...
        if len(sig_summer) > 0:
            ...
            for _, row in sig_summer.iterrows():
                ...
                print(
                    f"  {row['instrument']} {row['fixed']} vs {row['dynamic']}: "
    ...
    winter_valid = winter[(winter["fixed_n"] >= MIN_TRADES) & (winter["dynamic_n"] >= MIN_TRADES)]
    ...
    if winter_valid.empty:
        ...
        if len(sig_winter) > 0:
            print(f"\nWARNING: {len(sig_winter)} significant winter difference(s) — unexpected if times truly align:")
    ...
    for fixed_label, dynamic_label, dst_col, event_desc in SESSION_PAIRS:
        for instrument in ACTIVE_INSTRUMENTS:
            ...
            if (
                summer_row["fixed_n"] < MIN_TRADES
                or summer_row["dynamic_n"] < MIN_TRADES
                or winter_row["fixed_n"] < MIN_TRADES
                or winter_row["dynamic_n"] < MIN_TRADES
            ):
                continue
            ...
            print(
                f"  {instrument} {fixed_label} vs {dynamic_label}: "
    ...
    if n_summer_sig > 0:
        for _, row in summer_sig.iterrows():
            print(
                f"  - {row['instrument']} {row['fixed']} vs {row['dynamic']} summer: "
                f"delta={row['delta_r']:+.4f}R, p={row['p_val']:.4f}, "
