...
for instrument in ["MGC", "MNQ"]:
    ...
    for window_name, start in [("18m", START_18M), ("12m", START_12M)]:
        ...
        print(
            f"  {'Session':>8s} {'Break':>6s} {'Single':>7s} {'Dbl':>5s} {'Sgl%':>6s} {'Dbl%':>6s} | {'Sgl MFE':>8s} {'Dbl MFE':>8s} {'Sgl WR@2R':>10s} {'Dbl WR@2R':>10s}"
        )
        print(f"  {'-' * 90}")
        ...
        for orb in sorted(get_enabled_sessions(instrument)):
            ...
            print(
                f"  {orb:>8s} {total:>6d} {single:>7d} {dbl:>5d} {sgl_pct:>5.1f}% {dbl_pct:>5.1f}% | {sgl_mfe_s:>8s} {dbl_mfe_s:>8s} {sgl_wr_s:>10s} {dbl_wr_s:>10s}"
            )
    ...
    for orb in SESSIONS:
        ...
        print(f"\n  --- {orb} (N={len(df)}) ---")
        print(
            f"  {'Bucket':>8s} {'N':>5s} {'Sgl%':>6s} {'MFE':>7s} {'MAE':>7s} {'MFE/MAE':>8s} {'P90 MFE':>8s} {'>2R%':>6s} {'>3R%':>6s}"
        )
        print(f"  {'-' * 70}")
        ...
        for lo, hi, label in buckets:
            ...
            print(
                f"  {label:>8s} {len(sub):>5d} {sgl_pct:>5.1f}% {avg_mfe:>6.2f}R {avg_mae:>6.2f}R {ratio:>7.2f}x {p90_mfe:>7.2f}R {gt2:>5.1f}% {gt3:>5.1f}%"
            )
