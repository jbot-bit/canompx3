...
def compute_orb(
    ...
    rows = con.execute(
        "SELECT high, low, volume FROM bars_1m "
...
def fetch_post_orb_bars(
    ...
    return con.execute(
        "SELECT high, low, close FROM bars_1m "
        "WHERE symbol = ? AND ts_utc >= ? AND ts_utc < ? "
        "ORDER BY ts_utc",
...
def main():
    ...
    for instrument in ACTIVE_ORB_INSTRUMENTS:
        ...
        print(f"{instrument}: {len(trading_days)} trading days "
              f"({trading_days[0]} to {trading_days[-1]})")

        inst_trades = 0
        for td_idx, trading_day in enumerate(trading_days):
            if td_idx % 500 == 0 and td_idx > 0:
                print(f"  {instrument}: {td_idx}/{len(trading_days)} days, "
                      f"{inst_trades} trades")
            ...
            for aperture in APERTURES:
                bris_start = datetime(
                    ...
                    h_adj, m_adj, tzinfo=BRIS,
                )

                orb_high, orb_low, orb_vol = compute_orb(
                    con, instrument, bris_start, aperture,
                )
                ...
                post_bars = fetch_post_orb_bars(
                    con, instrument, bris_orb_end, MAX_BARS_POST_ORB,
                )
                if not post_bars:
                    continue
                ...
                for entry_model, cb in ENTRY_SPECS:
                    for rr in RR_TARGETS:
                        pnl_r_gross, break_dir = simulate_trade(
                            post_bars, orb_high, orb_low, rr, entry_model, cb,
                        )
    ...
    for key in sorted(diag_groups.keys()):
        ...
        print(f"{key[0]:<10} O{key[1]:<4} {key[2]:<8} {total:<7} "
              f"{dbl:<7} {rate:<7.1%} {avg_vol:<8.0f} {avg_range:<8.2f}")
    ...
    print(f"\nOverall adjacent double-break rate: {overall_dbl_rate:.1%} "
          f"(vs LONDON_METALS ~81%)")
    ...
    for r in all_results:
        for filter_name, min_size in G_FILTER_THRESHOLDS.items():
            if r.orb_range >= min_size:
                key = (r.instrument, r.aperture, r.entry_model, r.confirm_bars,
                       r.rr_target, filter_name)
    ...
    if n_survivors > 0:
        print(f"\n{'Cell':<50} {'N':>5} {'WR':>6} {'AvgR':>7} "
              f"{'$Avg':>8} {'$Tot':>9} {'Shrp':>6} {'p':>8}")
        print("-" * 105)
        for i, (key, metrics) in enumerate(cell_metrics):
            if i in significant:
                ...
                m = metrics
                print(f"{label:<50} {m['n']:>5} {m['wr']:>6.1%} "
    ...
    print(f"{'Cell':<50} {'N':>5} {'WR':>6} {'AvgR':>7} "
          f"{'$Avg':>8} {'Shrp':>6} {'p':>8} {'BH':>3}")
    print("-" * 100)
    ...
    if n_survivors > 0:
        ...
        print(f"{'Cell':<50} {'W_N':>5} {'W_AvgR':>7} "
              f"{'S_N':>5} {'S_AvgR':>7}")
        print("-" * 80)
        for i, (key, _metrics) in enumerate(cell_metrics):
            if i in significant:
                ...
                print(f"{label:<50} {len(winter):>5} {w_avg:>+7.3f} "
                      f"{len(summer):>5} {s_avg:>+7.3f}")
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "instrument", "aperture", "entry_model", "confirm_bars",
            "rr_target", "filter", "n", "win_rate", "avg_r_net", "total_r",
            "avg_dollars", "total_dollars", "sharpe", "p_value",
            "bh_significant",
        ])
        for i, (key, metrics) in enumerate(cell_metrics):
            inst, ap, em, cb, rr, filt = key
            m = metrics
            writer.writerow([
                inst, ap, em, cb, rr, filt,
                m["n"], f"{m['wr']:.4f}", f"{m['avg_r']:.6f}",
                f"{m['total_r']:.2f}", f"{m['avg_dollars']:.2f}",
                f"{m['total_dollars']:.2f}", f"{m['sharpe']:.4f}",
                f"{m['p_value']:.6f}",
    ...
    with open(trades_csv, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "trading_day", "instrument", "aperture", "entry_model",
            "confirm_bars", "rr_target", "pnl_r_gross", "pnl_r_net",
            "pnl_dollars", "orb_range", "break_dir", "double_break", "season",
        ])
        for r in all_results:
            writer.writerow([
                r.trading_day, r.instrument, r.aperture, r.entry_model,
                r.confirm_bars, r.rr_target, f"{r.pnl_r_gross:.6f}",
                f"{r.pnl_r_net:.6f}", f"{r.pnl_dollars:.2f}",
                f"{r.orb_range:.2f}", r.break_dir, r.double_break, r.season,
            ])
        ...
        for i, (key, metrics) in enumerate(cell_metrics):
            if i in significant:
                ...
                if m["n"] >= 100:
                    ...
                    cls = "INVALID"
                print(f"  {inst} O{ap} {em}_CB{cb} RR{rr} {filt}: "
    ...
    print(f"  Adjacent double-break rate: {overall_dbl_rate:.1%} "
          f"(LONDON_METALS correct time: ~81%)")
