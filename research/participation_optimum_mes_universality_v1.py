"""Participation-optimum universality test on MES — confirmatory of MNQ finding.

Instrument-swap of research/participation_optimum_universality_v1.py.
Same pooled quadratic regression, K=1 primary + K=22 per-cell secondary.

Pre-reg: docs/audit/hypotheses/2026-04-20-participation-optimum-mes-universality-v1.yaml
Lock SHA: 77e33f06
Parent MNQ verdict: CONFIRMED_UNIVERSAL (β₂=-0.00156, t=-5.189, 21/24 cells).
...
PREREG_PATH = "docs/audit/hypotheses/2026-04-20-participation-optimum-mes-universality-v1.yaml"
PREREG_SHA = "77e33f06"
RESULT_DOC = Path("docs/audit/results/2026-04-20-participation-optimum-mes-universality-v1.md")

INSTRUMENT = "MES"
SESSIONS = [
    "CME_PRECLOSE", "CME_REOPEN", "COMEX_SETTLE",
    "EUROPE_FLOW", "LONDON_METALS", "NYSE_CLOSE", "NYSE_OPEN",
    "SINGAPORE_OPEN", "TOKYO_OPEN", "US_DATA_1000", "US_DATA_830",
]
DIRECTIONS = ["long", "short"]

# Parent MNQ summary for cross-instrument table
...
def main() -> int:
    ...
    try:
        ...
        con.close()
    if len(data) == 0:
        print("VERDICT: SCAN_ABORT — no MES canonical data at 5m")
        return 1
    ...
    if not integrity_ok or pooled.n == 0:
        verdict, reason = "SCAN_ABORT", f"integrity fail: rel_vol non-null {rv_non_null:.1%}"
        ...
        reason = (f"Pooled beta2={pooled.beta2:+.5f} t={pooled.beta2_t:+.3f} p={pooled.beta2_p_one_tailed:.4f} "
                  f"does NOT clear t<=-3.0. MNQ finding does NOT replicate on MES — mechanism is MNQ-specific.")
    elif agreement < 0.25:
        verdict = "KILL_SIMPSON_MES"
        reason = f"Pooled significant but per-lane agreement {agreement:.1%} < 25% — Simpson's artefact."
    elif agreement < 0.50:
        verdict = "CONFIRMED_HETEROGENEOUS_MES"
        ...
        verdict = "CONFIRMED_UNIVERSAL_MES"
        reason = (f"Pooled beta2={pooled.beta2:+.5f} t={pooled.beta2_t:+.3f} significant; "
                  f"per-lane agreement {agreement:.1%} >= 50%. Mechanism replicates on MES.")

    RESULT_DOC.parent.mkdir(parents=True, exist_ok=True)
    parts = []
    parts.append(f"# Participation-optimum universality — MES v1\n")
    parts.append(f"**Pre-reg:** `{PREREG_PATH}` (LOCKED, commit_sha=`{PREREG_SHA}`)")
    parts.append(f"**Script:** `research/participation_optimum_mes_universality_v1.py`")
    parts.append(f"**Parent MNQ merged:** PR #41, commit `126ed6b8`")
    parts.append(f"**Scope:** MES × 11 active sessions × both directions × 5m E2 CB1 RR1.5 unfiltered, IS only")
    parts.append("")
    ...
    parts.append(f"- Cells with β₂ < 0: {len(neg)}/{len(valid)} = **{agreement:.1%}**")
    parts.append("")
    parts.append("## Not done by this result")
    parts.append("")
    parts.append("- No capital action.")
    parts.append("- Does NOT test MGC, 15m/30m, E3/E4, other RRs.")
    parts.append("- If CONFIRMED_*, unblocks a multi-instrument deployment-shape pre-reg; does not deploy on its own.")
    parts.append("")
