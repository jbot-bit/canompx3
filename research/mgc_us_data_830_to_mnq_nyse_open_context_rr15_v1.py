...
Scope is locked by:
  docs/audit/hypotheses/2026-04-22-mgc-us-data-830-to-mnq-nyse-open-context-rr15-v1.yaml

This runner evaluates exactly one pre-registered cross-asset chronology path
on the unfiltered MNQ NYSE_OPEN O5 E2 CB1 RR1.5 lane using canonical
...
PREREG_PATH = "docs/audit/hypotheses/2026-04-22-mgc-us-data-830-to-mnq-nyse-open-context-rr15-v1.yaml"
RESULT_DOC = Path("docs/audit/results/2026-04-22-mgc-us-data-830-to-mnq-nyse-open-context-rr15-v1.md")
ROW_CSV = Path("docs/audit/results/2026-04-22-mgc-us-data-830-to-mnq-nyse-open-context-rr15-v1-rows.csv")
...
def load_lane_df() -> pd.DataFrame:
    ...
    df["is_oos"] = df["trading_day"].dt.date >= HOLDOUT_SACRED_FROM
...
def _welch(on: pd.Series, off: pd.Series) -> tuple[float, float]:
    ...
    t_val, p_val = stats.ttest_ind(on.astype(float), off.astype(float), equal_var=False)
...
def _one_sample_t(values: pd.Series) -> tuple[float, float]:
    if len(values) < 2:
        return float("nan"), float("nan")
    t_val, p_val = stats.ttest_1samp(values.astype(float), 0.0)
    return float(t_val), float(p_val)
...
def _t6_null_floor(df: pd.DataFrame, b: int = 1000) -> tuple[float, float]:
    ...
    n_on = int(is_df["feature_fire"].sum())
    more_extreme = 0
    for _ in range(b):
