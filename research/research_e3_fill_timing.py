...
def run_audit(db_path: Path, threshold_min: int = DEFAULT_THRESHOLD_MIN) -> None:
    ...
    with duckdb.connect(str(db_path), read_only=True) as con:
        ...
        for session in sessions:
            ...
            print(f"\n{'-' * 60}")
            print(
                f"SESSION: {session}  (N_fills={len(session_unique)} unique, "
            ...
            for bucket_label, _, _ in TIME_BUCKETS:
                ...
                print(
                    f"  {bucket_label:<14} {fill_count:>6} {fill_pct:>6.1f}% {m['n']:>6} "
            ...
            print(
                f"  STALE (>={threshold_min}min): {len(stale_unique)} fills ({len(stale_unique) / total_unique * 100:.1f}%), "
            ...
            if len(rr_targets) > 1:
                ...
                for rr in rr_targets:
                    ...
                    print(
                        f"  {rr:>5.1f} {fr_m['n']:>8} {fr_m['avg_r'] if fr_m['avg_r'] is not None else 'N/A':>10} "
            ...
            if len(instruments) > 1:
                ...
                for inst in instruments:
                    ...
                    if fr_m["avg_r"] is not None and st_m["avg_r"] is not None:
                        delta = f"{st_m['avg_r'] - fr_m['avg_r']:+.4f}"
                    print(
                        f"  {inst:>5} {fr_m['n']:>8} {fr_m['avg_r'] if fr_m['avg_r'] is not None else 'N/A':>10} "
                        f"{st_m['n']:>8} {st_m['avg_r'] if st_m['avg_r'] is not None else 'N/A':>10} {delta:>8}"
                    )
            ...
            if stale_pct < 5:
                ...
                if stale_m["avg_r"] is not None and fresh_m["avg_r"] is not None:
                    ...
                    if delta > 0.1:
                        print(
                            f"  -> Stale 5-20% ({stale_pct:.1f}%), stale avgR BETTER by {delta:+.4f}R. FIX NEEDED (artificial inflation)."
                        )
                    else:
                        print(
                            f"  -> Stale 5-20% ({stale_pct:.1f}%), stale avgR delta={delta:+.4f}R. Document only (dilution, not inflation)."
                        )
                else:
        ...
        print(f"\n{'=' * 80}")
        print(
            f"OVERALL: {total} unique E3 fills, "
