...
def main():
    ...
    parser.add_argument("--target-dd", type=float, default=5.0, help="Target max drawdown in R (default: 5)")
    ...
    try:
        ...
        print(f"  {'Rank':>4} {'Slot':<25} {'Sharpe':>7} {'SlotDD':>7} {'Sh/DD':>7} {'ExpR':>7} {'N':>5}")
        print(f"  {'-' * 4} {'-' * 25} {'-' * 7} {'-' * 7} {'-' * 7} {'-' * 7} {'-' * 5}")

        for i, s in enumerate(ranked, 1):
            label = f"{s['instrument']}_{s['session']}"
            print(
                f"  {i:>4} {label:<25} {s['head_sharpe_ann']:>6.2f} "
        ...
        for top_n in range(2, len(ranked) + 1):
            ...
            print(f"  {top_n:>6} {m['total_r']:>+8.1f}R {sh:>8} {m['max_dd']:>6.1f}R {m['wr']:>5.1%}")
        ...
        print(
            f"  {'Slots':>6} {'Thr':>5} {'Scale':>6} {'TotalR':>9} {'Sharpe':>8} {'MaxDD':>7} {'WR':>6} {'ScalePct':>9}"
        )
        ...
        for r in sorted(results, key=lambda x: -x["metrics"]["total_r"]):
            ...
            print(
                f"  {r['slots']:>6} {r['threshold']:>4}R {r['scale']:>5.2f} "
        ...
        print(f"  {'#':>4} {'Slot':<25} {'Entry':>5} {'RR':>4} {'Filter':<15} {'Sharpe':>7} {'DD':>6} {'ExpR':>7}")
        ...
        for i, slot in enumerate(best["selected"], 1):
            ...
            print(
                f"  {i:>4} {label:<25} {em:>5} {rr:>4} {ft:<15} "
        ...
        print(f"  - When portfolio DD > {best['threshold']}R from peak: risk {best['scale']}R per trade")
        ...
        if bm["dd_start"] and bm["dd_end"]:
            print(f"    Period:        {bm['dd_start']} to {bm['dd_end']} ({bm['dd_duration']} cal days)")
            if bm["recovery"]:
        ...
        for year in sorted(yearly.keys()):
            ...
            print(f"  {year:>6} {y['n']:>7} {y['total_r']:>+8.1f}R {expr:>+9.4f} {wr:>6.1%}{marker}")
            if y["total_r"] < 0:
                all_pos = False
        ...
        for r_dollar in [100, 200, 300, 400, 500, 750, 1000]:
            ...
            monthly = annual / 12
            print(f"  ${r_dollar:>7} ${annual:>10,.0f} ${dd_dollar:>10,.0f} ${acct:>12,.0f} ${monthly:>10,.0f}")
        ...
        print(f"{'=' * 90}")
        print(f"  {'Target':>8} {'Slots':>6} {'Config':>20} {'TotalR':>9} {'Sharpe':>8} {'MaxDD':>7} {'Ann R':>9}")
        ...
        print(
            f"  {target_dd:>7.0f}R {best['slots']:>6} {config:>20} "
        ...
        for other_dd in [10, 15, 20]:
            ...
            if other_best:
                ...
                print(
                    f"  {other_dd:>7}R {other_best[0]:>6} {other_best[1]:>20} "
