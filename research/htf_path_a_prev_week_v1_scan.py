#!/usr/bin/env python3
"""HTF Path A prev-week v1 family scan (Pathway A, K_family=24).

Pre-registered at:
  docs/audit/hypotheses/2026-04-18-htf-path-a-prev-week-v1.yaml
...
Output: docs/audit/results/2026-04-18-htf-path-a-prev-week-v1-scan.md
...
YAML_PATH = PROJECT_ROOT / "docs/audit/hypotheses/2026-04-18-htf-path-a-prev-week-v1.yaml"
RESULT_PATH = PROJECT_ROOT / "docs/audit/results/2026-04-18-htf-path-a-prev-week-v1-scan.md"
...
def _predicate_sql(session: str, direction: str) -> str:
    ...
    return (
        f"d.orb_{session}_break_dir = 'short' AND d.prev_week_low IS NOT NULL AND d.orb_{session}_low < d.prev_week_low"
    )
...
def _build_cell(
    ...
    predicate = "pwh_break_long" if direction == "long" else "pwl_break_short"
...
def _emit_result_md(
    ...
    lines: list[str] = []
    lines.append("# HTF Path A prev-week v1 — Family Scan Results")
    lines.append("")
    ...
    lines.append(f"**Pre-registration:** `docs/audit/hypotheses/2026-04-18-htf-path-a-prev-week-v1.yaml`")
    lines.append("")
    lines.append("## Scope")
    lines.append("")
    ...
    lines.append(f"- Directions / cells: {list(DIRECTIONS)} (pwh_break_long / pwl_break_short)")
    ...
    lines.append(
        "- Look-ahead: `prev_week_*` populated by canonical "
        "`pipeline.build_daily_features._apply_htf_level_fields` from fully-closed "
        "prior Mon-Sun week only. Drift check 59 (HTF integrity) passes with "
    ...
    lines.append("")
    lines.append("```")
    lines.append(f"DUCKDB_PATH={db_path} python research/htf_path_a_prev_week_v1_scan.py")
    lines.append("```")
    lines.append("")
