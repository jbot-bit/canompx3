"""Generated startup read-model for minimal complete task understanding."""
...
from context.registry import TaskRoute, file_packets_for_paths, resolve_from_text, resolve_task
from pipeline.startup_index import load_startup_index
...
def _resolve_route(task_text: str | None, task_id: str | None) -> tuple[TaskRoute, list[BriefIssue]]:
    ...
    if task_text:
        ...
        if route is None:
            if candidates:
                issues.append(
                    BriefIssue("blocker", "ambiguous_route", "Mutating/system startup blocked: task routing is ambiguous.", ", ".join(candidate.task_id for candidate in candidates))
                )
            else:
                issues.append(BriefIssue("blocker", "missing_route", "Mutating/system startup blocked: no deterministic task route matched."))
...
def build_system_brief(
    ...
    *,
    ...
    touched_paths: list[str] | tuple[str, ...] = (),
    ...
    startup_index = load_startup_index(root)

    packet_paths = list(touched_paths) if touched_paths else list(route.canonical_files)
    packets = file_packets_for_paths(packet_paths)
    packet_payloads = [startup_index["file_packets"].get(packet.path, {"path": packet.path}) for packet in packets]
    ...
    for path in (*route.doctrine_files, *route.canonical_files):
        if not (root / path).exists():
            issue = {"level": "blocker", "code": "missing_owner_path", "message": "Required route path missing.", "detail": path}
    ...
    warnings.extend(
        {"level": "warning", "code": "missing_decision_ledger", "message": "Decision ledger missing or empty.", "detail": item}
