...
def test_build_scan_skips_generated_research_artifacts(git_repo: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    artifact_sha = _commit(
        git_repo,
        "artifacts/research/run_001/manifest.json",
        '{"status":"ok"}\n',
        "generated artifact",
    )
    _git(git_repo, "update-ref", "refs/remotes/origin/main", artifact_sha)
    monkeypatch.setattr(
        daily_bug_scan, "_verification_status", lambda _root: daily_bug_scan.VerificationStatus("static_only", "test")
    )
    ...
    assert any(item.sha == artifact_sha for item in packet.skipped_commits)
    skipped = next(item for item in packet.skipped_commits if item.sha == artifact_sha)
    assert skipped.reason == "doc-only/no production code"
    assert all(item.sha != artifact_sha for item in packet.candidate_commits)
