...
from trading_app.live.bot_dashboard import (
    ...
    _start_broker_connect_background,
    _strategy_meta,
)
...
def test_profile_session_ambiguity_warns_for_shared_session_profiles():
...
def test_api_alerts_returns_recent_runtime_alerts(monkeypatch):
    ...
    payload = asyncio.run(bot_dashboard.api_alerts(limit=20))
    assert len(payload["alerts"]) == 1
    assert payload["alerts"][0]["category"] == "feed_dead"
    assert payload["summary"]["counts"]["critical"] == 1


def test_build_operator_payload_filters_alerts_to_selected_profile(monkeypatch):
    seen = {}
    ...
    def _fake_collect_alert_summary(*, limit=25, profile=None, mode=None):
    ...
    monkeypatch.setattr(bot_dashboard, "_collect_alert_summary", _fake_collect_alert_summary)
    monkeypatch.setattr(
        bot_dashboard,
        "_profile_session_ambiguity",
        lambda profile_id: {"status": "pass", "detail": f"ok:{profile_id}"},
    ...
    assert payload["profile"] == "topstep_50k_mnq_auto"
    assert seen["profile"] == "topstep_50k_mnq_auto"


def test_start_broker_connect_background_runs_without_blocking():
    ...
    thread = _start_broker_connect_background(manager)
