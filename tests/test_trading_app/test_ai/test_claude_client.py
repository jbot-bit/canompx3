...
class TestGetClient:
    ...
    def test_get_client_without_sdk_raises_import_error(self):
        """Missing optional Anthropic SDK should fail closed with a clear message."""
        from trading_app.ai import claude_client

        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "sk-test-fake"}, clear=True):
            with patch.object(claude_client, "_load_anthropic", side_effect=ImportError("missing sdk")):
