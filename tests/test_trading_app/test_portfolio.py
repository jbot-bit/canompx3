    ...
    for s in strategies:
        ...
        """,
            [
                ...
                s["expectancy_r"],
                s.get("sharpe_ratio"),
                s.get("max_drawdown_r"),
                s.get("median_risk_points"),
                s.get("median_risk_points"),
                "{}",
            ],
        )
...
class TestDiversification:
    def test_max_strategies_enforced(self):
        ...
        candidates = [_make_strategy(strategy_id=f"s{i}", orb_label=orbs[i % len(orbs)]) for i in range(50)]
        selected = diversify_strategies(candidates, max_strategies=10)
        assert len(selected) == 10

    def test_max_per_orb_enforced(self):
        candidates = [_make_strategy(strategy_id=f"s{i}", orb_label="US_DATA_830") for i in range(20)]
    ...
    def test_max_per_entry_model(self):
        candidates = [
            _make_strategy(strategy_id=f"s_e1_{i}", entry_model="E1", expectancy_r=0.30 - i * 0.01) for i in range(10)
        ]
...
class TestSerialization:
    ...
    def test_summary_correct(self, tmp_path):
        strategies = [
            ...
            _make_strategy(
                strategy_id="s2", orb_label="LONDON_METALS", entry_model="E3", expectancy_r=0.20, win_rate=0.50
            ),
        ]
        db_path = _setup_db(tmp_path, strategies)
        portfolio = build_portfolio(db_path=db_path, instrument="MGC")
...
class TestBuildStrategyDailySeries:
    def test_shared_calendar_index(self, tmp_path):
        ...
        outcomes_a = [
            {
                "trading_day": df_rows[i]["trading_day"],
                "orb_label": "CME_REOPEN",
                "rr_target": 2.0,
                "confirm_bars": 5,
                "entry_model": "E1",
        ...
        outcomes_b = [
            {
                "trading_day": df_rows[200 + i]["trading_day"],
                "orb_label": "US_DATA_830",
                "rr_target": 2.0,
                "confirm_bars": 5,
                "entry_model": "E1",
    ...
    def test_eligible_no_trade_is_zero(self, tmp_path):
        ...
        outcomes = [
            {
                "trading_day": df_rows[0]["trading_day"],
                "orb_label": "CME_REOPEN",
                "rr_target": 2.0,
                "confirm_bars": 5,
                "entry_model": "E1",
        ...
        for i in range(min(50, len(col))):
            ...
            if size < 8.0:
                assert np.isnan(col.iloc[i]), (
                    f"Day {i} (size={size}) is ineligible for G8 but got {col.iloc[i]}; should be NaN"
                )

        # Verify skipped count is non-zero
        assert s["overlays_skipped_ineligible"] > 0, "Expected some overlays skipped due to ineligibility"
...
class TestCalendarOverlay:
    ...
    def test_nfp_day_nan_with_overlay(self, tmp_path):
        ...
        outcomes = [
            {
                "trading_day": df_rows[3]["trading_day"],
                "orb_label": "CME_REOPEN",
                "rr_target": 2.0,
                "confirm_bars": 5,
                "entry_model": "E1",
    ...
    def test_overlay_reduces_eligible_count(self, tmp_path):
        ...
        assert stats_cal[strat["strategy_id"]]["eligible_days"] == (stats_no[strat["strategy_id"]]["eligible_days"] - 5)
...
class TestNestedIntegration:
    ...
    def _setup_nested_db(
        self, tmp_path, baseline_strats, nested_strats, daily_features_rows, outcome_rows, nested_outcome_rows=None
    ):
    ...
    def test_build_strategy_daily_series_nested(self, tmp_path):
        ...
        outcomes_baseline = [
            {
                "trading_day": df_rows_5m[i]["trading_day"],
                "orb_label": "CME_REOPEN",
                "orb_minutes": 5,
                "rr_target": 2.0,
                "confirm_bars": 5,
        ...
        outcomes_nested = [
            {
                "trading_day": df_rows_15m[i]["trading_day"],
                "orb_label": "CME_REOPEN",
                "orb_minutes": 15,
                "entry_resolution": 5,
                "rr_target": 2.0,
        ...
        series_df, stats = build_strategy_daily_series(
            db_path,
            [
                baseline[0]["strategy_id"],
                nested[0]["strategy_id"],
            ],
        )
...
class TestCorrelationFilter:
    ...
    def test_none_corr_matrix_backward_compatible(self):
        ...
        candidates = [
            _make_strategy(strategy_id=f"s{i}", orb_label="CME_REOPEN", expectancy_r=0.30 - i * 0.01) for i in range(5)
        ]
...
class TestFamilyHeadsOnly:
    ...
    def test_returns_only_heads(self, tmp_path):
        ...
        strategies = [
            _make_strategy(strategy_id="MGC_CME_REOPEN_E1_RR2.0_CB2_ORB_G5", orb_label="CME_REOPEN", expectancy_r=0.30),
            _make_strategy(strategy_id="MGC_CME_REOPEN_E1_RR2.5_CB2_ORB_G5", orb_label="CME_REOPEN", expectancy_r=0.25),
            _make_strategy(strategy_id="MGC_CME_REOPEN_E1_RR2.0_CB2_ORB_G8", orb_label="CME_REOPEN", expectancy_r=0.40),
        ]
        db_path = _setup_db(tmp_path, strategies)
