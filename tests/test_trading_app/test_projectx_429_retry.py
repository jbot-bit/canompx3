...
As of 2026-05-18 the retry path lives in BrokerHTTPClient. Tests patch
requests.Session.request and `time.sleep` (now in http_client module) to
keep wall-clock low.
"""
...
def _mock_resp(status_code: int = 200, json_data: dict | list | None = None) -> MagicMock:
    resp = MagicMock()
...
def test_cancel_raises_rate_limit_exhausted_on_429(mock_req, mock_sleep):
    ...
    mock_req.return_value = _mock_429()
    router = _router()
    with pytest.raises(RateLimitExhausted):
        router.cancel(order_id=42)
    # At least 3 attempts (ORDER_POLICY.max_429_attempts=3, max_attempts=4)
    assert mock_req.call_count >= 3
...
@patch("trading_app.live.http_client.time.sleep")
@patch("requests.Session.request")
def test_verify_bracket_legs_returns_none_on_non_429_error(mock_req, mock_sleep):
    """Non-429 errors in query_open_orders → (None, None) as before.

    BrokerHTTPClient retries class-A/B/C/D, so a stable ConnectionError will
    raise BrokerTransientError after exhaustion. verify_bracket_legs catches
    that as `except Exception` and returns (None, None) — same observable
    contract.
    """
