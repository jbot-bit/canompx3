...
class TestRandomWalkForwardIntegrity:
    ...
    def test_oos_trade_count_matches_outcomes(self):
        ...
        Walk-Forward Artifact Requirements:
        ------------------------------------
        These tests validate walk-forward validation results against independent
        recomputation from orb_outcomes.

        What generates walk_forward_results.json:
        - Generated by: trading_app/strategy_validator.py (Phase 6 validation)
        - Command: `python trading_app/strategy_validator.py --instrument MGC`
        - Prerequisites:
          * Validated strategies in gold.db:validated_setups table
          * Full orb_outcomes dataset covering validation date range
          * Requires Phase 1-5 completion (discovery → validation filtering)

        When artifact is expected:
        - After running full validation pipeline for at least one instrument
        - After strategies pass Phase 1-5 filters (min_sample, sharpe, stability)
        - After walk-forward analysis completes (20-day train, 5-day test folds)
        - In CI environments: Should be pre-built or skipped gracefully

        Why tests require it:
        - Walk-forward validation is the FINAL guard against overfitting
        - These tests verify no train/test leakage in date ranges
        - Validates that OOS metrics match actual trade counts in test folds
        - Ensures fold aggregation (sum of folds == total OOS) is correct
        - Catches silent corruption in walk-forward result storage

        How to generate walk_forward_results.json:
        1. Complete pipeline: `python pipeline/run_pipeline.py --instrument MGC`
        2. Build outcomes: `python trading_app/outcome_builder.py --instrument MGC`
        3. Run discovery: `python trading_app/strategy_discovery.py --instrument MGC`
        4. Run validator: `python trading_app/strategy_validator.py --instrument MGC`
        5. Artifacts written to: <project>/artifacts/walk_forward/walk_forward_results.json

        Data Limitation Note:
        - Walk-forward validation requires 20-day training windows
        - Limited by available data depth (2021-2026 for MGC)
        - Strategies with sparse trade days may have insufficient data for validation
        - This is expected behavior for highly filtered strategies (G6/G8)
        """
        _skip_if_no_db()
        # Check for walk-forward artifact generated by strategy_validator.py Phase 6
        wf_json = Path(__file__).parent.parent / "artifacts" / "walk_forward" / "walk_forward_results.json"
        if not wf_json.exists():
            # SKIP: Artifact not present. Run strategy_validator.py to generate.
            # See docstring above for full reproduction steps.
    ...
    def test_no_train_test_overlap_in_results(self):
        ...
        Walk-Forward Artifact Requirements:
        ------------------------------------
        This test validates walk-forward fold integrity — specifically that no
        fold has train/test date overlap (which would constitute data leakage).

        What generates walk_forward_results.json:
        - Generated by: trading_app/strategy_validator.py (Phase 6 validation)
        - Each fold contains: train_start, train_end, test_start, test_end
        - Validation logic enforces: train_end < test_start (no leakage)

        When artifact is expected:
        - After completing Phase 1-5 validation filtering (discovery → stability)
        - After walk-forward analysis completes for validated strategies
        - Requires sufficient data depth for 20-day train + 5-day test windows

        Why tests require it:
        - Walk-forward validation is meaningless if folds overlap
        - This test is the CRITICAL guard against train/test leakage
        - Validates that validator.py correctly enforces causal ordering
        - Prevents future regressions to lookahead bias in fold construction

        How to generate walk_forward_results.json:
        1. Run full validation pipeline (see test_oos_trade_count_matches_outcomes docstring)
        2. Validator writes fold metadata with date ranges to JSON artifact
        3. This test independently verifies date range integrity

        Note: If this test fails (not skips), walk-forward validation is CORRUPT
        and all OOS metrics are invalid. This is a FAIL-CLOSED design.
        """
        # Check for walk-forward artifact generated by strategy_validator.py Phase 6
        wf_json = Path(__file__).parent.parent / "artifacts" / "walk_forward" / "walk_forward_results.json"
        if not wf_json.exists():
