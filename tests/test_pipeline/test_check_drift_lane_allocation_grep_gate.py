...
class TestNoDirectLaneAllocationJsonLiterals:
    ...
    def test_allowlisted_temporary_site_passes(self, tmp_path, monkeypatch):
        ...
        # opportunity_awareness.py is in the temporary allowlist.
        _write_py(
            tmp_path,
    ...
    def test_dead_temporary_allowlist_entry_violates(self, tmp_path, monkeypatch):
        ...
        # opportunity_awareness.py is in temporary allowlist. Create the file
        ...
        assert "trading_app/opportunity_awareness.py" in violations[0]
    ...
    def test_mutation_probe_removing_allowlist_entry_surfaces_site(
        ...
        # opportunity_awareness.py with the literal — should pass under
        ...
        mutated = frozenset(
            ...
            if p != Path("trading_app/opportunity_awareness.py")
        )
        monkeypatch.setattr(
        ...
        violations = check_no_direct_lane_allocation_json_literals()
        assert len(violations) == 1
        assert "trading_app/opportunity_awareness.py:1" in violations[0]
        assert "resolve_allocation_json" in violations[0]
