...
Usage:
    ...
    python scripts/ingest_mnq.py --resume
    python scripts/ingest_mnq.py --dry-run
...
LOG_FILE = PROJECT_ROOT / "scripts" / "ingest_mnq.log"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MNQ full pipeline (local disk)")
    ...
    rc = run_step("Ingest MNQ bars_1m", ingest_cmd)
    if rc != 0:
