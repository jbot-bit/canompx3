...
FINDING_TYPE_RULES: list[tuple[str, str]] = [
    ...
    (r"async.?safety|blocking.*async|return_exceptions|shared.*mutable.*lock", "async_safety"),
    (r"state.?persist|crash.*lose|not.*written.*disk|_save_state|persist.*gap", "state_persistence"),
    (r"contract.?drift|signature.*mismatch|caller.*wrong.*arg|removed.*kwarg", "contract_drift"),
