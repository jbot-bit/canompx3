...
from trading_app.config import STOP_MULTIPLIERS  # noqa: E402
...
def _supported_stop_multiplier(stop_multiplier: float) -> bool:
    return any(abs(stop_multiplier - float(supported)) < 1e-9 for supported in STOP_MULTIPLIERS)
...
def plan_replay_work(
    ...
    *,
    ...
    for candidate in selected:
        ...
        if not _supported_stop_multiplier(stop_multiplier):
            planned.append(
                ReplayWorkItem(
                    rank=candidate.rank,
                    strategy_id=candidate.strategy_id,
                    factory_status="BLOCKED_UNSUPPORTED_STOP",
                    next_action=(
                        f"Unsupported stop_multiplier={stop_multiplier:g}; add a canonical stop model and tests "
...
def build_prereg_draft(candidate: FactoryCandidate, *, today: str) -> dict[str, Any]:
    ...
    stop_multiplier = float(parse_stop_multiplier(strategy_id))
    return {
        "metadata": {
            "theory_grant": False,
        ...
        "hypotheses": [
            {
                ...
                "role": {
                    "kind": "standalone",
                    "parent": (f"Exact {strategy_id} canonical lane replay on orb_outcomes x daily_features."),
                    "comparator": (
        ...
        "execution_gate": {
            ...
            "operator_must_review_before_flip": [
                ...
                "confirm stop_multiplier support via the canonical strict runner",
...
def parse_result_md(path: Path) -> ParsedResult:
    ...
    if sample_size is None:
        sample_size = _int_or_none(
            _match_optional(text, r"\*\*IS sample size:\*\*\s*`?(\d+)`?") or _match_optional(text, r"\bN=(\d+)\b")
        )
    if exp_r is None:
        exp_r = _float_or_none(
            _match_optional(text, r"\*\*IS ExpR:\*\*\s*`?([-+]?[0-9]*\.?[0-9]+)`?")
            or _match_optional(text, r"\bExpR=([-+]?[0-9]*\.?[0-9]+)")
        )
    if t_stat is None:
        t_stat = _float_or_none(_match_optional(text, r"\*\*IS t-stat:\*\*\s*`?([-+]?[0-9]*\.?[0-9]+)`?"))
    return ParsedResult(
        path=path,
        strategy_id=strategy_id,
        verdict=verdict,
...
def _audit_log_verdict(result: ParsedResult) -> str:
    verdict = result.verdict
    if verdict in {"PASS_CHORDIA_OOS_UNDERPOWERED", "PASS_PROTOCOL_A_OOS_UNDERPOWERED"}:
        return "PARK"
    if verdict == "FAIL_STRICT_CHORDIA":
        if result.t_stat is not None and result.t_stat < 3.0:
            return "FAIL_BOTH"
        return "FAIL_CHORDIA"
    return verdict
    ...
    for result in parsed_results:
        proposals.append(
            AuditLogProposal(
                ...
                verdict=_audit_log_verdict(result),
...
def render_report(
    ...
    *,
    ...
    lines = [
        ...
        "- unsupported stop lanes are blocked; supported non-default stops use the canonical tight-stop transform",
        "",
        "## Work Items",
        "",
