...
def print_profile(name, sessions):
    ...
    print(f"  {'-' * 55}")
    print(
        f"  TOTAL: {len(sessions)} sessions | risk=${total_risk} ({total_risk / DD * 100:.0f}% DD) | ev=+${total_ev:.1f}/day | ${annual:,.0f}/yr"
    )
    return total_ev
...
print("  P(all 8 lose same day): < 0.4% (independent sessions)")
