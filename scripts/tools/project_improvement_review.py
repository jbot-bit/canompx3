...
NO_GO_REOPEN_RE = re.compile(r"\b(NO[- ]GO|NOGO|graveyard|dead)\b.{0,120}\b(reopen|revive|retry)\b", re.IGNORECASE | re.DOTALL)
PRE_REG_RE = re.compile(r"\b(pre[- ]?reg|pre_registered|K accounting|expected_trial_count|total_expected_trials)\b", re.IGNORECASE)
TRADING_HYPOTHESIS_RE = re.compile(r"\b(hypothesis|strategy|filter|threshold|entry_model|ORB|OOS)\b", re.IGNORECASE)
RESEARCH_CLAIM_RE = re.compile(r"\b(research shows|literature says|studies show|memory says|from memory)\b", re.IGNORECASE)
LOCAL_SOURCE_RE = re.compile(r"\b(docs/institutional/literature/|resources/|arxiv\.org|doi\.org|https?://)\b", re.IGNORECASE)
...
def _changed_since_ref(root: Path, since_ref: str) -> tuple[tuple[str, ...], str | None]:
    rc, out, err = _run_git(root, ["diff", "--name-only", f"{since_ref}..HEAD"])
    if rc != 0:
        return (), err or out or f"git diff failed for {since_ref}"
...
def gather_git_state(root: Path, since_ref: str = DEFAULT_SINCE_REF) -> GitState:
    ...
    changed, changed_err = _changed_since_ref(root, since_ref)
    if changed_err:
...
def _scan_research_integrity(path: str, text: str) -> list[Finding]:
    ...
    if _is_reviewer_own_surface(path):
        return findings
    is_research_surface = path.startswith(("research/", "docs/audit/", "docs/institutional/", "chatgpt_bundle/")) or path.endswith(
        ("RESEARCH_RULES.md", "STRATEGY_BLUEPRINT.md")
    )
    ...
    if is_research_surface and TRADING_HYPOTHESIS_RE.search(text) and "hypothesis" in text.lower() and not PRE_REG_RE.search(text):
        findings.append(
            _finding(
                "Research integrity",
                "MEDIUM",
...
def _scan_code_quality(path: str, text: str) -> list[Finding]:
    ...
    if path.endswith(".py"):
        ...
        if db_match and READ_ONLY_TOKEN not in text:
            findings.append(
                _finding(
                    "Code quality",
                    "HIGH",
...
def _scan_protected_mutation(path: str, text: str) -> list[Finding]:
    ...
    for token in PROTECTED_MUTATION_HINTS:
        for match in re.finditer(re.escape(token), text, re.IGNORECASE):
            ...
            if re.search(r"\b(write|dump|apply|mutate|deploy|sync|upsert|delete|go_live|go-live)\b", window, re.IGNORECASE):
                hits.append(token)
                break
    if not hits:
        return []
...
def _git_findings(git_state: GitState) -> list[Finding]:
    ...
    protected_staged = sorted(
        ...
        if any(token.lower() in path.lower() for token in ("lane_allocation", "live_config", "prop_profiles", "pinecone"))
    )
    if protected_staged:
        findings.append(
            _finding(
    ...
    review_artifacts = [path for path in git_state.untracked_files if "review" in path.lower() or path.startswith("docs/runtime/")]
    if review_artifacts:
        findings.append(
            _finding(
                "Git/worktree hygiene",
...
def render_markdown(report: ReviewReport) -> str:
    ...
    lines.append("```")
    lines.append("")
    lines.append("> Report-only output. Static pattern risks are not semantic proof; verify with targeted commands before claiming fixes.")
    return "\n".join(lines) + "\n"
