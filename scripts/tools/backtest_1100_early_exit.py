...
def run_backtest(db_path=None):
    ...
    for _, trade in outcomes.iterrows():
        ...
        for threshold in thresholds:
            ...
            if mtm_points < 0:
                ...
                pnl_r = round(to_r_multiple(cost, entry_price, stop_price, mtm_points), 4)
    ...
    for rr in [1.5, 2.0, 2.5, 3.0]:
        for cb in [3, 4, 5]:
            baseline_sub = [r for r in results["baseline"] if r["rr_target"] == rr and r["confirm_bars"] == cb]
            ...
            for t in thresholds:
                sub = [r for r in results[t] if r["rr_target"] == rr and r["confirm_bars"] == cb]
    ...
    exit_cb4_rr25 = [r for r in results[best_t] if r["rr_target"] == 2.5 and r["confirm_bars"] == 4]
    ...
    for rr in [2.0, 2.5]:
        baseline_sub = [r for r in results["baseline"] if r["rr_target"] == rr and r["confirm_bars"] == 4]
        exit_sub = [r for r in results[best_t] if r["rr_target"] == rr and r["confirm_bars"] == 4]
    ...
    for min_size, flabel in [(4.0, "G4+"), (6.0, "G6+"), (8.0, "G8+")]:
        baseline_sub = [
            ...
            if r["rr_target"] == 2.5 and r["confirm_bars"] == 4 and r["orb_size"] >= min_size
        ]
        exit_sub = [
            r for r in results[best_t] if r["rr_target"] == 2.5 and r["confirm_bars"] == 4 and r["orb_size"] >= min_size
        ]
...
def _compare_header():
    print(f"  {'Label':<14} {'N':>5} {'EarlyX':>6} {'WR':>6} {'ExpR':>7} {'Sharpe':>7} {'TotalR':>8} {'MaxDD':>7}")
    print("  " + "-" * 62)
...
def _print_comparison(label, records):
    ...
    dd = (cum - peak).min()
    print(
        f"  {label:<14} {len(pnl):>5} {n_early:>6} {wr:>5.1%} {expr:>+7.3f} "
...
def _yearly_comparison(baseline, exit_data, threshold):
    ...
    for year in years:
        ...
        print(
            f"  {year:<6} {len(b_pnl):>4} {n_early:>6} "
            f"{b_wr:>6.1%} {e_wr:>6.1%} "
