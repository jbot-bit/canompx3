...
def assert_no_live_session() -> None:
    ...
    lock) and refuse to open a gold.db write connection while any live PID or
    ambiguous pre-PID lock may be active. This enforces the repo invariant "NEVER run two write processes
    ...
    for lock_path in _LOCK_DIR.glob("bot_*.lock"):
        ...
        if locked:
            holder = f"live PID {pid}" if pid is not None else f"ambiguous lock ({reason})"
            raise RuntimeError(
