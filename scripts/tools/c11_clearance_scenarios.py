...
def _lane_trades(con, lane_defs, eff_stop):
    ...
    for lane in lane_defs:
        ...
        out[sid] = _load_lane_trade_paths(
            con, sid, as_of_date=AS_OF, effective_stop_multiplier=eff_stop.get(sid)
        )
    return out
...
def _scenarios_for_subset(con, instruments, trades_by_lane, subset_ids):
    ...
    return [
        _scenario_from_trade_paths(d, [t for t in trades_by_day.get(d, []) if t.strategy_id in subset_ids])
        for d in cal
    ]
...
def main():
    ...
    eff_stop = {
        L.strategy_id: float(L.planned_stop_multiplier or profile.stop_multiplier) for L in lane_specs
    }
    rules = _with_consistency_rule(_build_rules(profile), profile)
    instruments = sorted({l["instrument"] for l in lane_defs})
    all_ids = [l["strategy_id"] for l in lane_defs]

    print(f"PROFILE {PROFILE_ID} | daily_loss_limit=${rules.daily_loss_limit} | "
    ...
    try:
        ...
        for sid, trs in trades_by_lane.items():
            ...
            print(f"  {sid}: trades={len(trs)} trade_days={len(dd)} "
        ...
        for bd in gfull["breach_days"]:
            ...
            print(f"  {bd}: total_close=${sc.total_pnl_dollars:,.0f} intraday_min=${sc.min_balance_delta_dollars:,.0f} "
        ...
        for sid in all_ids:
            ...
            standalone[sid] = g
            print(f"  {sid}: gate_pass={g['gate_pass']} strict={g['strict_pass']} op={g['operational_pass']} "
        ...
        for drop in all_ids:
            ...
            print(f"  drop {drop.split('_')[1]:13}: gate_pass={g['gate_pass']} strict={g['strict_pass']} "
        ...
        for pair in combinations(all_ids, 2):
            ...
            print(f"  {tags:28}: gate_pass={g['gate_pass']} strict={g['strict_pass']} "
                  f"op={g['operational_pass']} breach={g['breach_count']} max90dd=${g['max_90d_dd']:,.0f}/${g['budget']:,.0f}")
