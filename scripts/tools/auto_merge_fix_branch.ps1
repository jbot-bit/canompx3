...
try {
    ...
    if ($behind -ne '0' -or $mergeBase -ne $originMain) {
        ...
        if ($dirtyLines.Count -gt 0) {
            Write-Log "[skip] auto-rebase requested but worktree has allowed dirty paths; stash or clear them first: $($dirtyLines -join '; ')"
            exit 0
        }
        Write-Log "[go] auto-rebasing fix branch onto origin/main (ahead=$ahead behind=$behind)"
        if (-not $DryRun) {
            $rebaseResult = Invoke-Git $Worktree @('rebase', 'origin/main') -AllowFailure
            if ($rebaseResult.Code -ne 0) {
                Invoke-Git $Worktree @('rebase', '--abort') -AllowFailure | Out-Null
                Write-Log "[skip] auto-rebase failed; conflict/manual merge required: $($rebaseResult.Output)"
                exit 0
            }
        }
        Invoke-Git $Worktree @('fetch', 'origin') | Out-Null
        ...
        $ahead = (Invoke-Git $Worktree @('rev-list', '--count', 'origin/main..HEAD')).Output
        if ($behind -ne '0' -or $mergeBase -ne $originMain) {
            Write-Log "[skip] branch still not a clean fast-forward after auto-rebase (ahead=$ahead behind=$behind)"
            exit 0
        }
    }
