#!/usr/bin/env python3
"""Read-only cross-session context audit.

Tests whether earlier same-day session state conditions later-session breakout
quality as continuation or reversal context.
"""
...
HYP_PATH = ROOT / "docs" / "audit" / "hypotheses" / "2026-04-11-cross-session-context-audit.yaml"
N_PERM = 5000
MIN_GROUP_N = 20
ORB_MINUTES = 5
...
def load_trials(path: Path) -> list[Trial]:
    ...
    for h in data["hypotheses"]:
        scope = h["scope"]
        prior_session = str(h["filter"]["prior_session"])
...
def assert_feature_safety(trial: Trial) -> None:
    ...
    unsafe = [c for c in (prior_dir_col, prior_outcome_col) if not is_feature_safe(c, trial.session)]
    if unsafe:
        raise ValueError(
...
def apply_filter(df: pd.DataFrame, filter_type: str) -> pd.Series:
    ...
    if filter_type == "PRIOR_WIN_ALIGN":
        return (df["prior_outcome"] == "win") & same_dir
    if filter_type == "PRIOR_LOSS_OPPOSED":
