...
outcome_pivot = outcomes.pivot_table(index="trading_day", columns="orb_label", values="pnl_r", aggfunc="first")

df = df_rows.merge(outcome_pivot, left_on="trading_day", right_index=True, how="inner", suffixes=("", "_out"))
...
for target in US_SESSIONS:
    ...
    if r:
        ...
        print(
            f"  {target} PDH: T({r['n_true']},{r['expr_true']:.4f}) F({r['n_false']},{r['expr_false']:.4f}) sp={r['spread']:.4f} p={r['p_value']:.4f}"
        )
    ...
    if r2:
        ...
        print(
            f"  {target} PDL: T({r2['n_true']},{r2['expr_true']:.4f}) F({r2['n_false']},{r2['expr_false']:.4f}) sp={r2['spread']:.4f} p={r2['p_value']:.4f}"
        )
...
for pred_s in PRE_US:
    ...
    for target in US_SESSIONS:
        ...
        if r:
            all_results.append(r)
            print(
                f"  {pred_s} -> {target}: Dbl({r['n_true']},{r['expr_true']:.4f}) No({r['n_false']},{r['expr_false']:.4f}) sp={r['spread']:.4f} p={r['p_value']:.4f}"
            )
...
for target in SESSIONS_ORDERED:
    ...
    r = binary_test(cond[sub_mask].astype(bool), out[sub_mask], f"H5: Gap*Dir -> {target}", yrs[sub_mask])
    if r:
        all_results.append(r)
        print(
            f"  {target}: Aligned({r['n_true']},{r['expr_true']:.4f}) Opposed({r['n_false']},{r['expr_false']:.4f}) sp={r['spread']:.4f} p={r['p_value']:.4f}"
        )
...
for pred_s, (c5, c15, c30) in apt_map.items():
    ...
    aligned = np.array([(a is not None and a == b and b == c) for a, b, c in zip(d5, d15, d30, strict=False)])
    ...
    for target in US_SESSIONS:
        ...
        if r:
            all_results.append(r)
            print(
                f"  {pred_s} -> {target}: Aligned({r['n_true']},{r['expr_true']:.4f}) Not({r['n_false']},{r['expr_false']:.4f}) sp={r['spread']:.4f} p={r['p_value']:.4f}"
            )
...
for pred_s in PRE_US:
    ...
    for target in US_SESSIONS:
        ...
        # MFE quintiles
        r = quintile_test(df[mfe_col].values.astype(float), out, f"H7: {pred_s}_MFE -> {target}", yrs)
        if r:
            all_results.append(r)
            qs = r["expr_per_q"]
            qk = sorted(qs.keys())
            print(
                f"  {pred_s} MFE -> {target}: Q1={qs[qk[0]]} Q5={qs[qk[-1]]} sp={r['spread']:.4f} p={r['p_value']:.4f}"
            )

        # MAE quintiles
        r2 = quintile_test(df[mae_col].values.astype(float), out, f"H7: {pred_s}_MAE -> {target}", yrs)
        if r2:
            all_results.append(r2)
            qs2 = r2["expr_per_q"]
            qk2 = sorted(qs2.keys())
            print(
                f"  {pred_s} MAE -> {target}: Q1={qs2[qk2[0]]} Q5={qs2[qk2[-1]]} sp={r2['spread']:.4f} p={r2['p_value']:.4f}"
            )
    ...
    for target in US_SESSIONS:
        ...
        if r:
            ...
            qk = sorted(qs.keys())
            print(
                f"  {pred_s} MFE/MAE -> {target}: Q1={qs[qk[0]]} Q5={qs[qk[-1]]} sp={r['spread']:.4f} p={r['p_value']:.4f}"
            )
...
for rank, idx in enumerate(sorted_idx):
    ...
    print(
        f"{rank + 1:>2} {r['label']:<52} {r['spread']:>7.4f} {r['p_value']:>7.4f} {adj_pvals[idx]:>7.4f} {fdr:>4} {n:>5}"
    )
...
for h_prefix, h_name in hypothesis_groups.items():
    h_tests = [(i, all_results[i]) for i in range(n_tests) if all_results[i]["label"].startswith(h_prefix)]
    if not h_tests:
        print(f"\n{h_prefix}: {h_name}")
    ...
    print(
        f"  Best: {best_r['label']}  spread={best_r['spread']:.4f}  p={best_r['p_value']:.4f}  p_BH={adj_pvals[best_idx]:.4f}"
    )
    if fdr_any:
