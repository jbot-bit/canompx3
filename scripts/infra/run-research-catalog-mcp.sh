...
if [[ -x "$VENV_PY" ]]; then
  ...
  echo "ERROR: no usable Python found for research-catalog MCP." >&2
...
if ! "$PYTHON_BIN" -c "import fastmcp" >/dev/null 2>&1; then
  echo "ERROR: research-catalog MCP requires the 'fastmcp' package, but it is not installed in the selected interpreter." >&2
...
cd "$ROOT"
exec "$PYTHON_BIN" scripts/tools/research_catalog_mcp_server.py
