...
if [[ "$ROOT" == "$DEFAULT_ROOT" && "${CANOMPX3_ALLOW_SHARED_ROOT_MUTATION:-0}" != "1" ]]; then
  DEFAULT_WORKSTREAM="${CANOMPX3_DEFAULT_CODEX_WORKSTREAM:-$(generate_workstream_name codex)}"
  PURPOSE="${CANOMPX3_WORKSTREAM_PURPOSE:-Build / edit}"
  exec env \
    CANOMPX3_WORKSTREAM_PURPOSE="$PURPOSE" \
    "$DEFAULT_ROOT/scripts/infra/codex-worktree.sh" open "$DEFAULT_WORKSTREAM" -- "$@"
fi
