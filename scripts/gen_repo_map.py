...
Usage:
    python scripts/gen_repo_map.py          # Regenerate REPO_MAP.md
    python scripts/gen_repo_map.py --check  # Exit 1 if REPO_MAP.md is stale
"""
...
SCAN_DIRS = ["pipeline", "trading_app", "scripts", "tests"]
...
def generate_repo_map(root: Path) -> str:
    ...
    sections.append("")
    sections.append("Auto-generated by `scripts/gen_repo_map.py`. Do not edit manually.")
    sections.append("")
...
def main() -> None:
    ...
    if args.check:
        if not output_path.exists():
            print("REPO_MAP.md does not exist. Run: python scripts/gen_repo_map.py")
            sys.exit(1)
        existing = output_path.read_text(encoding="utf-8")
        if existing.rstrip() != content.rstrip():
            print("REPO_MAP.md is stale. Run: python scripts/gen_repo_map.py")
            sys.exit(1)
