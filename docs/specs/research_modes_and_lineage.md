...
| Source | Page | Verified content |
|---|---|---|
| **Chan, Ernest P. (2013). *Algorithmic Trading: Winning Strategies and Their Rationale*. Wiley. ISBN 978-1-118-46014-6.** | **p25** | *"[W]e must perform a walk-forward test as a final, true out-of-sample test. This walk-forward test can be conducted in the form of paper trading, but, even better, the model should be traded with real money (albeit with minimal leverage) so as to test those aspects of the strategy that eluded even paper trading. **Most traders would be happy to find that live trading generates a Sharpe ratio better than half of its backtest value.**"* — extracted verbatim from `resources/Algorithmic_Trading_Chan.pdf` p25. This is the **practitioner 50% halving convention** applied to the Sharpe ratio. **Caveat on metric equivalence:** Chan's halving applies to Sharpe (= mean_excess_return / stdev), while our WFE is OOS_ExpR / IS_ExpR (from `walkforward.py:310-313`). If OOS stdev differs materially from IS stdev, Sharpe halving and ExpR halving transform to different WFE values. The two halvings are therefore the **same conservative halving convention**, not an algebraic equivalence. Chan's citation supports the 0.5 value as a widely-held practitioner convention but does not substitute for a direct WFE-specific threshold derivation. Chan is a Wiley-published quant with a PhD and a commercially active quant fund. |
...
- `resources/Lopez_de_Prado_ML_for_Asset_Managers.pdf` (45pp): **REVISED 2026-04-07 in self-review — the initial characterization of this PDF as "bibliography only" was wrong.** The actual structure is: pp 1-5 front matter, **pp 6-28 Chapter 1 "Introduction" (main text, 23 pages)**, pp 29-41 Bibliography/References, pp 42-45 back matter. Chapter 1 contains substantive backtest-overfitting discussion directly relevant to the WFE grounding. **Verbatim quotes:**
  - **MLAM Ch 1 p7:** *"Contrary to popular belief, backtesting is not a research tool. Backtests can never prove that a strategy is a true positive, and they may only provide evidence that a strategy is a false positive. Never develop a strategy solely through backtests. Strategies must be supported by theory, not historical simulations."*
  - **MLAM Ch 1 p13:** *"the backtest–tweak–backtest cycle is a futile exercise that will inevitably end with an overfit strategy (a false positive)"*
  - **MLAM Ch 1 p14:** *"This is the approach followed by the combinatorial purged cross-validation method, or CPCV (AFML, chapter 12)"* — explicit reference to CPCV as the methodological alternative to walk-forward. CPCV body text is in AFML Ch 12 which we do not have locally, but MLAM Ch 1 confirms its existence and purpose.
  - **MLAM Ch 1 p16:** *"Section 8 explains the dangers of backtest overfitting, and provides several practical solutions to the problem of selection bias under multiple testing"* — Section 8 body is not in our local excerpt but is pointed to as the PBO-related treatment.
  - **Relevance to WFE decision:** MLAM Ch 1 reinforces the institutional-rigor posture (theory-first, backtest-as-falsification-tool, CPCV as modern alternative). It does NOT prescribe a different WFE threshold, so the § 9.3 decision is unaffected. It does STRENGTHEN the grounding chain by adding a second peer-reviewed Lopez de Prado source (alongside Harvey-Liu 2015 JPM, Bailey-Lopez de Prado DSR 2014, and Bailey-Borwein-Lopez de Prado-Zhu Pseudo-mathematics 2014).
  - **The one "Walk Forward" grep hit at p34 is** a bibliography entry for Żbikowski (2015), not a definition — that specific initial observation was correct, but "bibliography only" as a characterization of the whole PDF was wrong.
...
Revision history:
- First pass (commit `609b29c`) framing was "PARTIAL but defensible"
- Second pass (commit `4f212ee`) upgraded after identifying that `backtesting_dukepeople_liu.pdf` is actually **Harvey & Liu (2015) "Backtesting" in Journal of Portfolio Management** — a peer-reviewed academic primary source
- **Third pass (this commit) corrects a self-caught skim error: `Lopez_de_Prado_ML_for_Asset_Managers.pdf` is NOT bibliography-only. It contains Chapter 1 (pp 6-28) with substantive backtest-overfitting discussion including an explicit CPCV reference.**

The final local grounding chain is:
...
2. **Peer-reviewed academic secondary (Lopez de Prado MLAM 2020, Ch 1 — local):** explicit statement at p7 that "backtesting is not a research tool" and that strategies "must be supported by theory, not historical simulations"; p13 critiques the "backtest–tweak–backtest cycle" as "a futile exercise that will inevitably end with an overfit strategy"; p14 references CPCV (from AFML Ch 12, not locally extractable) as the methodological alternative to walk-forward. Lopez de Prado is a Cambridge University Press / Wiley author, Guggenheim Partners Head of Quantitative Trading & Research, and one of the most-cited living authors in quantitative finance.
3. **Academic-practitioner secondary citing Pardo (Aronson 2007):** cites Pardo 1992 by name as the canonical walk-forward source, describes the methodology, and explicitly states *"there is no theory that suggests what fraction of the data should be assigned to training and testing ... often it's a seat-of-the-pants call"* (p339). Honestly characterizes walk-forward partition choices as practitioner convention, not theoretical law.
4. **Practitioner convention (Chan 2013):** *"Most traders would be happy to find that live trading generates a Sharpe ratio better than half of its backtest value"* (p25). The same 50% halving convention applied to Sharpe (not algebraically identical to our ExpR-ratio WFE but the same conservative posture).
5. **Modern statistical alternatives (Bailey-Lopez de Prado 2014 DSR + Bailey-Borwein-Lopez de Prado-Zhu 2014 Pseudo-mathematics / PBO):** provide the strict-superiority alternatives (DSR with selection-bias correction, PBO via CSCV). Both already in local resources. These papers use "walk" and "half" tokens in their bodies but in contexts that are mostly *random walks* (Monte Carlo) and *half-life* (mean reversion), not walk-forward or 50% discount — grep hits on those tokens should be read in context.
6. **Empirical state:** all 124 active strategies pass `wfe >= 0.5` comfortably (min = 0.5141). Zero current cost.
...
5. **Peer-reviewed academic grounding + practitioner convention.** The 0.5 threshold is cited in code comments and `RESEARCH_RULES.md:59` with a multi-source grounding chain, in order of authority: (a) **Harvey & Liu (2015) "Backtesting" in *Journal of Portfolio Management*** — peer-reviewed primary source for the 50% discount convention, with mathematical derivation via multiple-testing correction. Our BH FDR pipeline is already aligned with Harvey-Liu's principled approach. (b) **Lopez de Prado (2020) *Machine Learning for Asset Managers*, Cambridge University Press, Ch 1 pp 6-28** — reinforces the theory-first / backtest-as-falsification-tool posture; explicit CPCV reference as methodological alternative. (c) **Aronson (2007) *Evidence-Based Technical Analysis* p339** — academic-practitioner source stating that the exact partition threshold has "no theory" (characterizing the 0.5 value as practitioner convention, not theoretical law). (d) **Chan (2013) *Algorithmic Trading* p25** — practitioner corroboration of the 50% halving, applied to Sharpe not ExpR (same conservative posture, not an algebraic equivalence — see § 9.2 caveat). (e) **Pardo 2008** is the original historical source but not locally extractable; its absence is non-blocking because Harvey-Liu provides the peer-reviewed justification and MLAM Ch 1 adds a second peer-reviewed Lopez de Prado voice. No new research is needed to defend the current 0.5 gate.
