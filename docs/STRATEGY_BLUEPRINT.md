...
| NR4/NR7 (Narrow Range) filter | DEAD | Crabel pattern. NR4 fires 50%, NR7 fires 33% on 23h futures (vs ~14% equities). Lift direction flips across ALL 9 instrument×RR combos. 2/96 BH FDR survivors (MES CME_REOPEN RR2.0 t=4.40, MNQ EUROPE_FLOW RR1.0 t=3.56) but both contradicted by negative-lift sessions. Session-specific NR7 (ORB size instead of daily range, fire rate 15.6% — closer to Crabel target) also tested: 0/96 BH FDR survivors. Holdout: MES CME_REOPEN direction holds (N=42) but MNQ EUROPE_FLOW REVERSES. E2/CB1, RR{1.0,1.5,2.0}, O15, pre-holdout. `verify_external_ibs_nr7.py`. Apr 2026 | No viable reopen path identified. Standard NR7 structurally wrong (33% fire rate). Session-range NR7 tested and failed (0/96 BH). |
| IBS (Internal Bar Strength) filter | DEAD | Prior RR2.0-only test missed the signal. Corrected multi-RR test (192 tests, K=192): 97 BH FDR survivors in-sample. Strongest: MNQ CME_PRECLOSE O15 RR1.0 Q1 t=6.78, cross-confirmed MES t=6.02. Continuous r(IBS,PnL)=-0.23 at CME_PRECLOSE O15. Not confounded with OVNRNG (Jaccard 0.037). **HOLDOUT KILLS:** Q1 reverses to -0.52R (MNQ, N=12) and -0.44R (MES, N=13) — both instruments reverse from strongly positive IS. Direction flips across sessions (Q1>Q4 at CME_PRECLOSE but Q4>Q1 at NYSE_OPEN/TOKYO_OPEN). Aperture flips (O5 momentum vs O15 mean-reversion). E2/CB1, RR{1.0,1.5,2.0}, O{5,15}. `verify_external_ibs_nr7.py`. Apr 2026 | Holdout consistency when more data accumulates (currently N=12 per quartile in OOS). Not a specification gap — the signal exists at RR1.0 but doesn't hold forward. |
...
| VWAP breakout direction gate | PROMISING (upgraded from observation) | 204 tests (2 VWAP definitions x 2 apertures x 3 RR x 3 inst x 6 sessions). 4/425 cross-family BH FDR survivors: MNQ US_DATA_1000 O15 RR1.5 (p=0.0002) + RR2.0 (p=0.00008), MNQ CME_PRECLOSE O5 RR1.0 (p=0.002) + RR2.0 (p=0.002). Not confounded with gap (r=-0.011), overnight_range_pct (r=-0.017), or atr_vel_ratio (r=-0.023). orb_mid and break_price definitions both produce survivors in different sessions. E2/CB1. Apr 2026 | Pre-register hypothesis for MNQ US_DATA_1000 (orb_mid VWAP gate, O15) and MNQ CME_PRECLOSE (break_price VWAP gate, O5). Run through full discovery pipeline with holdout enforcement. |
