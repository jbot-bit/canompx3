...
| NR4/NR7 (Narrow Range) filter | DEAD | Crabel pattern. NR4 fires 50%, NR7 fires 33% on 23h futures (vs ~14% equities). Lift direction flips across ALL 9 instrument×RR combos. 0/85 BH FDR survivors (corrected for 3x N-inflation from daily_features join). Session-specific NR7 (ORB size instead of daily range, fire rate 15.6%) also tested: 0/96 BH FDR survivors. E2/CB1, RR{1.0,1.5,2.0}, O15, pre-holdout. `verify_external_ibs_nr7.py`. Apr 2026 | No viable reopen path. Standard NR7 structurally wrong (33% fire rate). Session-range tested and failed. |
| IBS (Internal Bar Strength) filter | DEAD | Prior RR2.0-only test missed the signal. Corrected multi-RR test (192 tests, K=192): 34 BH FDR survivors in-sample. Strongest: MNQ CME_PRECLOSE O15 RR1.0 Q1 t=3.89 (N=74), cross-confirmed MES t=3.45 (N=44). Not confounded with OVNRNG (Jaccard 0.037). **HOLDOUT KILLS:** Q1 reverses to -0.52R (MNQ, N=12) and -0.44R (MES, N=13) — both instruments reverse from strongly positive IS. Direction flips across sessions (Q1>Q4 at CME_PRECLOSE but Q4>Q1 at NYSE_OPEN/TOKYO_OPEN). Aperture flips (O5 momentum vs O15 mean-reversion). E2/CB1, RR{1.0,1.5,2.0}, O{5,15}. `verify_external_ibs_nr7.py`. Apr 2026. **Note:** initial run had 3x N-inflation from missing orb_minutes=5 guard on daily_features CTE — corrected. | Holdout consistency when more data accumulates (currently N=12 per quartile in OOS). Signal exists at RR1.0 but doesn't hold forward. |
...
| Gap fill during ORB window = skip | DEAD | CME micros trade 23h → median overnight gap 0.50pts (MNQ), 0.25pts (MES). 101 tests (3 inst x 6 sessions x 3 RR x 2 apertures), 0/101 BH FDR survivors. No signal at any gap threshold (1/5/10/20pt). Gap fill rate ~50% = coin flip. Continuous: Spearman(|gap|, pnl_r) rho=0.02-0.07 — negligible. Same structural killer as gap-direction NO-GO. E2/CB1, O5+O15, RR1.0-2.0. Cross-family K=425: 0 survivors. Apr 2026 | Instrument with real overnight gaps (equities, not 23h futures) |
| Vol-spike avoidance (skip high atr_vel_ratio) | DEAD (reversed) | Fitschen (p91-101) thesis: high vol = noise = more stop-outs. Exhaustive: 18 tests (3 inst x 3 RR x 2 apertures), 7/18 BH FDR (6 at cross-family K=425). MES Q4>Q1 in ALL 8 years (2019-2026). Permutation: MES p=0.000, MGC p=0.004, MNQ p=0.044. **Mechanism decomposition: MAE (stop-outs) IDENTICAL between high/low vol (p=0.72 MNQ, 0.74 MES), but MFE (winner extension) significantly HIGHER in high vol (p=0.025 MNQ, 0.0003 MES, <0.0001 MGC).** Fitschen "more stop-outs" thesis specifically wrong for ORB — high vol = winners extend further, not more adverse excursion. Confirms ATR_VEL_GE105 TAKE direction. Apr 2026 | Nothing — direction robustly confirmed across instruments, apertures, years, and permutation. The mechanism is clear: high vol = MFE expansion, not MAE expansion. |
| Pre-velocity edge (binary aligned-vs-opposed) | DEAD | Cross-session descriptive K=89, 0 BH-FDR survivors. PR #119 (`docs/audit/results/2026-04-21-pre-break-context-cross-session-descriptive.md`). 9 raw-p<0.05 vs 4.45 Poisson-expected = within noise. Apr 2026 | New mechanism class — Fitschen continuation at hourly+ timescales (different from 5-bar pre-ORB) or microstructure-feature alternatives |
| Pre-velocity × atr_vel_regime interaction (Chan stop-cascade reframe) | DEAD | Same K=89 cross-section descriptive, alignment × regime interaction one-sided D-in-D Welch. K_within=79 (10 cells dropped under stricter 4-sub-cell power floor), K_cumulative_with_PR119=168, 0 BH-FDR survivors at any K. Top |t|=1.86 (MES EUROPE_FLOW O15) below Chordia 3.0. Direction-consistent (24/25 top cells positive interaction = mechanism-predicted) but statistically null. Pre-reg `docs/audit/hypotheses/2026-04-26-pre-velocity-vol-regime-interaction-descriptive.yaml`. Result `docs/audit/results/2026-04-26-pre-velocity-vol-regime-interaction-descriptive.md`. Apr 2026 | Continuous atr_vel_ratio quintile interaction (more power than binary regime split) AND a different mechanism citation (Chan Ch 7 stop-cascade now twice exhausted). Or Lopez_de_Prado_ML_for_Asset_Managers / Aronson 2007 extracts adding new structural priors. |
| Narrow relative IB width (compression-take) | DEAD (reversed, MES signal persists) | Crabel compression thesis adapted to session-specific ORB. 102 tests (size-controlled G5+ subset), 19/102 BH FDR (16 at cross-family K=425). Wide ORB OUTPERFORMS narrow even controlling for absolute size. MES dominates: 14/16 cross-family survivors are MES (CME_PRECLOSE, TOKYO_OPEN, COMEX_SETTLE, EUROPE_FLOW). Works on BOTH O5 and O15 for MES. MNQ: p=0.074 size-controlled (not independently significant). Relative width correlated rho=0.67 with absolute size — partially redundant with G4/G5/G6 but NOT fully explained by them on MES. Apr 2026 | MES-specific relative-width filter could be tested as hypothesis-scoped. Compression-take direction remains dead universally. |
| VWAP breakout direction gate | PROMISING (upgraded from observation) | 204 tests (2 VWAP definitions x 2 apertures x 3 RR x 3 inst x 6 sessions). 4/425 cross-family BH FDR survivors: MNQ US_DATA_1000 O15 RR1.5 (p=0.0002) + RR2.0 (p=0.00008), MNQ CME_PRECLOSE O5 RR1.0 (p=0.002) + RR2.0 (p=0.002). Not confounded with gap (r=-0.011), overnight_range_pct (r=-0.017), or atr_vel_ratio (r=-0.023). orb_mid and break_price definitions both produce survivors in different sessions. E2/CB1. Apr 2026 | Pre-register hypothesis for MNQ US_DATA_1000 (orb_mid VWAP gate, O15) and MNQ CME_PRECLOSE (break_price VWAP gate, O5). Run through full discovery pipeline with holdout enforcement. **Note: CME_PRECLOSE branch closed DEAD 2026-04-18 — see separate row below.** |
| VWAP_BP_ALIGNED on MNQ CME_PRECLOSE O5 (specific cell) | DEAD (OOS direction reversal under Mode A) | Pre-reg `docs/audit/hypotheses/2026-04-13-mnq-vwap-cme-preclose.yaml`. Result doc `docs/audit/results/2026-04-18-vwap-bp-cmepreclose-c8-recheck.md`. C8 re-check 2026-04-18 (commit `284e3d35`): all 3 RR cells at N_OOS=29/29/28. IS +0.167/+0.168/+0.162 (t=5.35/3.93/3.04), OOS −0.156/−0.354/−0.496 (t=−0.88/−1.79/−2.39). dir_match FAIL, eff_ratio −0.93/−2.11/−3.06 on all 3 cells. Fails Amendment 2.7 Criterion 8. Parent VWAP family (US_DATA_1000 O15) unaffected — those 3 lanes remain deployed. Apr 2026 | Fundamentally new mechanism explaining a session-level regime shift post-2026-01-01 on CME_PRECLOSE. NOT a RR / aperture / stop-multiplier / CB variant. |
| MNQ WIDE_REL_1.0X_20D × ORB_G5 at O5 | NULL (distinctness survives Rule 7, primary pass unreachable) | Pre-reg v2 `docs/audit/hypotheses/2026-04-18-mnq-wide-rel-ib-v2.yaml` (v1 withdrawn — OOS-shaped scope + wrong Rule 7 metric). Distinctness audit `docs/audit/results/2026-04-18-mes-wide-ib-distinctness-audit.md`. Replay result `docs/audit/results/2026-04-18-mnq-wide-rel-ib-v2-replay.md`. Replay commit `e1cc2977`: K=6 cells (CME_PRECLOSE + TOKYO_OPEN × RR{1.0,1.5,2.0}), 0/6 pass primary. Rule 7 fire correlation distinct (0.13-0.55 < 0.70 threshold). All cells fail Chordia t ≥ 3.79 (max t=2.74). BH-FDR K=6 passes 2 cells (CME_PRECLOSE_RR1.0, TOKYO_OPEN_RR1.0) but both fail on t and/or N_OOS and/or WFE. NO-GO Apr 13 entry claimed 14/16 MES-dominant survivors at K=425 O5+O15 pooled — did NOT reproduce at O5-only with proper Welch t-test + BH-FDR K=8. Apr 2026 | Fundamentally new mechanism. NOT a window-length sweep (10d/30d), threshold sweep (1.2x/1.5x), O15 aperture extension, or MES re-audit — those are Stage 2 variations pre-registered and blocked as reopens. |
| CROSS_NYSE_MOMENTUM on MNQ US_DATA_1000 + NYSE_CLOSE at O5 | NULL (LEAKAGE_SUSPECT upper guard fired on US_DATA_1000) | Pre-reg `docs/audit/hypotheses/2026-04-18-mnq-cross-nyse-momentum.yaml`. Distinctness audit `docs/audit/results/2026-04-18-mnq-cross-nyse-momentum-distinctness.md`. Replay result `docs/audit/results/2026-04-18-mnq-cross-nyse-momentum-replay.md`. Replay commit `16cc3af5`: K=6 cells, 0/6 pass primary. Rule 7 fire rho distinct on US_DATA_1000 vs X_MES_ATR60 (−0.024); NYSE_CLOSE has no deployed alternatives. All cells fail BH-FDR K=6 AND Chordia t ≥ 3.79 (max t=2.15). US_DATA_1000 (3/3 cells) tripped LEAKAGE_SUSPECT upper guard (eff_ratio ≤ 3.00): observed 3.50 / 7.44 / 287.15 on RR 2.0/1.5/1.0 respectively — near-zero IS baselines amplify OOS. NYSE_CLOSE (3/3 cells) fail N_OOS ≥ 30 (15-22 trades). Apr 11 memo `docs/plans/2026-04-11-cross-session-state-round3-memo.md` "strongest surviving branch" claim (IS +0.140 on TAKE_WIN_ALIGN) did NOT reproduce under Mode A: current RR1.0 IS delta = +0.0006. Apr 2026 | Fundamentally new mechanism. NOT VETO-side filter class, O15 aperture, different prior session, or 5-fold WFE rescue — those are Stage 2 variations and blocked as reopens. |
| OVNRNG_100 on MNQ NYSE_OPEN + US_DATA_1000 at O5 | DEAD (already REJECTED in validator pipeline) | Scanned in `experimental_strategies` 2026-04-11 (`n_trials_at_discovery=35616`). NYSE_OPEN RR1.0/1.5: IS p=0.133/0.272 — rejected Phase 3 year stability (3/6 and 4/6 years positive, <75% threshold). US_DATA_1000 RR1.0: IS p=0.005 passed BH FDR but rejected Criterion 8 (OOS ExpR −0.009, N_oos=61). US_DATA_1000 RR1.5: IS p=0.046, rejected Criterion 8 (OOS ExpR −0.107, N_oos=60). Status confirmed 2026-04-18 in `docs/audit/results/2026-04-18-overnight-adjacent-family-status.md` (commit `66d26edb`). Apr 2026 | Fundamentally new mechanism or genuine regime change on NYSE_OPEN / US_DATA_1000 post-2026. NOT an aperture swap (O15) or RR addition (RR2.0). |
