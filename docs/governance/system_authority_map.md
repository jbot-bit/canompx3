...
- Audits must verify the linked source, not restate their own local version of
  the rule.
- Plans and handoffs may explain decisions, but they do not become runtime
  truth.
...
| Canonical registries | Stable code truth for changing facts and rules | `pipeline/asset_configs.py`, `pipeline/cost_model.py`, `pipeline/dst.py`, `trading_app/config.py`, `trading_app/holdout_policy.py`, `trading_app/prop_profiles.py` | One owned source per concept; no duplicate literals downstream |
...
| Published read models / contracts | Stable query surfaces for operational consumers | `pipeline/db_contracts.py`, `trading_app/validated_shelf.py`, DB views `active_validated_setups` and `deployable_validated_setups` | Readers consume these instead of rebuilding semantics ad hoc |
...
| Reference / generated docs | Orientation aids and generated inventory | `docs/ARCHITECTURE.md`, `docs/MONOREPO_ARCHITECTURE.md`, `REPO_MAP.md` | Must be marked non-authoritative and kept linked/generated |
...
- The repo now has real contract modules for holdout policy, validated shelf,
  profile lanes, and lifecycle state.
- Drift, integrity, and behavioral audits exist and are useful.
- Published DB views now back part of the operational surface.

Still weak:

- `scripts/tools/` mixes commands, reports, audits, research helpers, and
  migrations in one drawer.
- Some audits still depend on deprecated or local truth instead of canonical
  runtime authorities.
- Reference docs like `docs/MONOREPO_ARCHITECTURE.md` and
  `docs/ARCHITECTURE.md` can drift into pseudo-authority unless explicitly
  marked and maintained.
- Many `validated_setups` readers still use raw table semantics because their
  role has not yet been classified.

## Enforcement Rules

1. New mutable workflow surfaces must declare their category here or in
   `docs/governance/document_authority.md` in the same change.
2. If a consumer needs deployable shelf semantics, it should read
   `deployable_validated_setups` or `deployable_validated_relation(...)`, not
   `validated_setups WHERE status='active'`.
3. If a rule changes frequently with data, profiles, or runtime state, do not
   hardcode it in prose. Link the source or expose a published contract.
4. Audits should fail when they read deprecated truth surfaces after a newer
