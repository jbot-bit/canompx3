...
| L4 | **Portfolio construction** | `trading_app/lane_allocation.py` → `docs/runtime/lane_allocation.json` | `lane_allocation.json` (`staleness` field) | adding a lane without checking correlation / EV vs the existing book — the literal tunnel | `strategy-lab get_lane_allocation_summary` (read `staleness`) |
...
| Q10 | Improves **portfolio** EV after correlation + drawdown constraints? | portfolio | `lane_allocation.py` / lane summary |
