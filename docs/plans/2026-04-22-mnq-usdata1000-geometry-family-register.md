...
## Transferred surviving family rows

### 5. `PD_CLEAR_LONG` on `COMEX_SETTLE RR1.0`
...
- exact meaning:
  - ORB midpoint outside prior-day congestion regime
  - not strictly inside prior-day range
  - not within `0.50 ATR-20` of the prior-day pivot
- current state:
  - discovery + validator passed
  - promoted to `validated_setups`
- key metrics:
  ...
  - `oos_exp_r=+0.2212`
- transfer note:
  - `COMEX_SETTLE` wants `PD_CLEAR_LONG`
  - it does **not** want the broader `PD_GO_LONG` union in the same way
...
Next honest move is another unsolved MNQ parent lane, not more cuts on
`US_DATA_1000` and not blind rollout of `PD_GO_LONG`.
