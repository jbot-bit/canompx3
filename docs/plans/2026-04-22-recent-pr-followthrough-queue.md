...
| MNQ NYSE_CLOSE RR1.0 role audit | `docs/runtime/stages/mnq-nyse-close-rr10-role-audit.md` | **ACTIONED.** Executed in `docs/audit/results/2026-04-23-mnq-nyse-close-rr10-role-audit.md`. Verdict: **CONTINUE as allocator candidate**. With the current 6 live MNQ lanes and `max_slots=7`, raw `NYSE_CLOSE RR1.0` improves common-window IS book math as a free-slot add (`annualized R +9.5`, honest Sharpe `+0.136`, candidate-to-book daily corr `+0.023`). This closes the role question without reopening filter search. |
...
|---:|---|---|---:|---:|---|
| 1 | Prior-day Pathway-B bridge execution triage | The branch no longer lacks locks: exact MNQ prior-day bridge hypotheses are now saved, but the repo still does not say which already-locked path should be executed next | 3 | 2 | Highest-signal way to turn the saved prior-day bridge package into one honest confirm-or-kill execution instead of leaving it as dormant scaffolding |
| 2 | Cross-asset earlier-session → later-ORB quality chronology spec | Still blocked on chronology discipline; no narrow spec written yet | 3 | 3 | Plausible, but easier to get wrong than the items above |
| 3 | GC→MGC 15m/30m translation question | Audit explicitly said no honest statement exists yet for wider apertures | 3 | 4 | Real question, but not until the 5m payoff-compression path is resolved |
...
- `NYSE_CLOSE` filter follow-through and role follow-through have now both been run to closure. The exact native `ORB_G8` prereg is killed, and the broad raw RR1.0 family survives specifically as a free-slot allocator candidate.
...
6. `docs/runtime/stages/mnq-nyse-close-rr10-role-audit.md` — now executed via `docs/audit/results/2026-04-23-mnq-nyse-close-rr10-role-audit.md`
...
1. triage and execute one already-locked prior-day bridge path instead of reopening that branch broadly,
2. write the chronology spec before any cross-asset timing scan,
3. leave `MNQ NYSE_CLOSE` out of filter-shopping mode and handle it, if at all, as a free-slot allocator candidate.
