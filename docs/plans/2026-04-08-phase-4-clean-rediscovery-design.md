---
title: Phase 4 — Clean Rediscovery (Mode A) Design
date: 2026-04-08
status: APPROVED after self-audit pass (gaps/errors/flaws fixed in place)
mode: plan → implementation (Stage 4.0 first)
parent_plan: docs/plans/2026-04-07-canonical-data-redownload.md
authority:
  - docs/institutional/pre_registered_criteria.md (12 locked criteria, Amendment 2.7)
  - docs/audit/hypotheses/README.md (hypothesis registry rules)
  - trading_app/holdout_policy.py (Mode A canonical source)
  - .claude/rules/research-truth-protocol.md (Phase 0 mandatory grounding)
  - .claude/rules/institutional-rigor.md (no band-aids, no skipping)
audit_pass:
  - first_draft: 2026-04-08 (pre-audit)
  - self_audit: 2026-04-08 (10 gaps/errors/flaws identified and fixed in place)
  - blast_radius_scout: 2026-04-08 (7 additional scope + test-regression issues flagged; incorporated)
  - critical_fixes:
    - "Gap 1: discovery scope limitation from hypothesis file (was post-facto check, now pre-facto enumeration constraint)"
    - "Gap 2: Criterion 8 (2026 OOS) gate was missing from Stage 4.0 scope"
    - "Gap 3: single-use hypothesis file rule was missing (re-runs would silently inflate multiple-testing)"
    - "Scout S1: verify_trading_app_schema expected_cols set must be updated in lockstep with ALTER TABLE (catches silent-pass test regressions)"
    - "Scout S2: existing UPDATE statements at validator lines 1102 and 1382 must populate rejection_reason (otherwise Stage 4.4 per-criterion breakdown impossible)"
    - "Scout S3: Criterion 8 must be N/A-safe when no OOS data available (otherwise test_integration.py + test_integration_l1_l2.py + revalidate_null_seeds.py all regress on synthetic pre-2026 data)"
    - "Scout S4: nested/regime validators bypass new gates — documented as intentional (experimental/non-production scope)"
    - "Scout S5: BOTH schema columns (hypothesis_file_sha + rejection_reason) added in Stage 4.0 — not split across 4.0/4.1 — because verify_trading_app_schema must know about them together"
    - "Scout S6: dependency sequence trap resolved — NULL hypothesis_file_sha on post-cutoff row → Criterion 1 fires first → Criterion 5 never reached → no DSR pre-committed-N ambiguity"
    - "Scout S7: Criterion 8 SQL must include orb_minutes in the join (triple-join trap prevention — use compute_dst_split pattern from validator line 314)"
---
