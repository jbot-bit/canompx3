...
- Headline result: STRICT Dalton 80% Rule fails on most MGC/MNQ/MES anchors (hit rates 3–50%, mostly 20–30%); LOOSE-touch hit rates are inflated by ambiguity (60–100%); MNQ 0900 anchor showed positive train-2024 uplift (+0.97R, ON N=48 / OFF N=635) and positive 2025-2026 mixed-test uplift (+0.46R, ON N=42 / OFF N=864) — but **WR collapses from 79% (train) to 43% (test), a single-split regime shift hidden by the headline ExpR**, and the script's "test" block mixes 2025 (Mode A IS) with 2026 (Mode A sacred OOS) per `HOLDOUT_SACRED_FROM=2026-01-01`, so per `research-truth-protocol.md` § "Mode B grandfathered baselines" the +0.46R cannot be cited as Mode A OOS evidence.
