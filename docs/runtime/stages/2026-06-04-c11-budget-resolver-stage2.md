...
mode: IMPLEMENTATION
status: IMPLEMENTED — pending adversarial audit (Tier B capital-path gate)
implemented: 2026-06-04 — 4 edits landed in working tree (NOT yet committed):
  - prop_profiles.py: added typed `self_imposed_dd_dollars: float|None=None` field;
    set =3_000.0 on self_funded_tradovate (promoted from notes "-$3,000 DD halt").
  - account_survival.py: resolver self-funded branch reads self_imposed_dd_dollars
    (risk-first SOURCE), fail-CLOSES to express belt if missing/≤0.
  - check_drift.py: upgraded check_prop_caps_do_not_leak_into_self_funded with
    structural sub-check (d) — executes resolver, flags self-funded budget that
    equals tier.max_dd at EITHER fraction (injection-verified: catches $6,000 and
    $5,400 leaks; passes clean).
  - test_account_survival.py: budget now sourced from $3,000 (not prop $6,000);
    added fail-closed-(b) self-funded-no-source test. 22 passed.
verification: 22 tests pass; full drift 176/0; ruff clean; resolver values
  confirmed by execution (self-funded $3,000, express $1,800 UNCHANGED — C11 still
  FAILS at $2,038, no profile flipped to PASS).
adversarial_audit: evidence-auditor dispatched (Tier B gate). The audit's attack
  on malformed inputs surfaced a REAL bug, FIXED before claim-of-done:
  `self_imposed_dd_dollars=True` resolved to a $1.00 budget because `bool` is an
  `int` subclass (isinstance(True,int) and True>0). Hardened the guard with an
  explicit `not isinstance(_, bool)` exclusion; added a regression test asserting
  True/False/NaN/0/neg/str all fail-close to the express belt ($5,400).
  Other audit vectors verified by execution and CLEAN: no caller bypasses the
  resolver (only the 3 in-resolver lines compute dd_limit_dollars*fraction);
  C11 verdict unchanged ($1,800); the new field-comment marker did not break drift
  check (b) (first marker line 139 < first self_funded tier line 604); injection
  catches both $6,000 and $5,400 leaks; passes clean.
next: commit (Tier A — local commit of verified work) is fine; merge/integration to
  the live truth-layer is the operator's call. C11 remains NO-GO — this stage only
  de-couples the self-funded budget source; it does NOT arm anything.
