...
2. **Check #173** `check_fast_lane_promote_queue_provenance_present` PASSED. Fail-closed on (a) any PROMOTE / QUEUED / SUPPRESSED_* entry in `promote_queue.yaml` lacking `structural_hash` / `k_lineage` / `N_hat`, (b) `rho_hat_assumed != 0.5` (locked prior), (c) banner scrub on the cache file, (d) STATUS_VALUES enum parity against this stage file's § Suppression Status Enum table.
...
5. **Check #173 injection tests** pass — (a) strip `structural_hash` from one entry, (b) NULL one `k_lineage` field, (c) missing `N_hat`, (d) hand-edit cache to mutate `rho_hat_assumed`, plus banner-scrub test and 6 sibling-coverage tests (one per SUPPRESSED_* token). All fail-closed.
6. **Self-review pass on live entry** `MNQ_US_DATA_1000_E1_RR1.0_CB2_PD_CLEAR_LONG_O30` — running the scanner against the live repo emits the entry with: `K_lane = 1` (no sibling retest yet), `structural_hash` present and reproducible, no graveyard match (verified against current digest), no E2 look-ahead (entry is E1), `N_hat` sane (a positive integer). Today the entry's terminal status is PARKED (`action-queue#20`) rather than QUEUED — that does not affect this AC: provenance fields must populate on every emitted entry regardless of terminal status, and PARKED is the canonical short-circuit upstream of the suppression chain (the suppression statuses only override `QUEUED` → `SUPPRESSED_*`). If any of K_lane / structural_hash / N_hat / rho_hat_assumed diverge, the design has a bug — not the entry.
...
1. Open this stage file (this commit only — establishes scope_lock + parent gate). **DONE** (commit `5cc63cc7`).
