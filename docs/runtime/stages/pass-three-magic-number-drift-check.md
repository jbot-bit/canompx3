...
created: 2026-04-26
updated: 2026-04-26
task: v6.1 Phase 5 Pass Three — magic-number rationale drift check on trading_app/live/. Closes the deferred-from-PR-#121 item by adding the IMPLEMENTATION stage that the guard requires for pipeline/check_drift.py edits.
---
...
- **Files modified:** `pipeline/check_drift.py` only — adds one new function `check_magic_number_rationale(trading_app_dir)` and one new entry to the `CHECKS` registry.
- **Drift count:** 113 → 114.
- **Production behavior:** zero impact. Drift checks are CI/pre-commit-only static analysis; do not run at trading-app runtime.
- **Existing magic numbers in `trading_app/live/`:** the new check evaluates on first run. Permissive rule — any comment containing "rationale" (case-insensitive) within ±5 lines passes. Most operational constants in `session_orchestrator.py` already have rationale comments (e.g., `FILL_POLL_TIMEOUT_SECS = 60.0` at line 2757 has 5 lines of explicit rationale).
...
For every UPPER_SNAKE_CASE assignment (class-body or module-level) in `trading_app/live/` whose value is a numeric literal with `abs(value) > RATIONALE_THRESHOLD` (default 10), require either:
  (a) a comment containing "Rationale:" or "rationale" (case-insensitive) within ±5 lines of the assignment, OR
