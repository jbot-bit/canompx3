...
- `cost-realism-slippage-pilot` — Book-wide cost-realism debt. **MGC, MNQ, and MES routine-central-tendency portions CLOSED:**
   ...
   - STILL OPEN: event-day tail NOT measured for MNQ/MES routine samples (known-unknown, not refuted concern). **MES event-tail debt is now bounded** via `docs/plans/2026-04-25-mes-event-tail-slippage-debt-scope.md`. Four escalation triggers named there: (A) any MES lane enters the live allocator (session-agnostic); (B) MES cost-model parameters change; (C) MES `max_contracts > 1`; (D) routine MES pilot is contradicted by a second sample. Pre-registered method: per-cell repricing across 4 event classes (calendar-event, macro-shock, liquidity-degraded, microstructure-burst), `reprice_e2_entry` repricer, `orb_utc_window` boundary, modeled-conservative bar = measured median `<= 1 tick` AND p95 `<= 2 ticks` against the canonical 5-ticks-per-side baseline. MNQ event-tail debt remains scoped at the same level by parity (no equivalent escalation-trigger doc authored yet — open follow-up).
