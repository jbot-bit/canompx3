...
def test_select_replay_candidates_blocks_non_default_stop_by_default() -> None:
    rows = [
    ...
    assert [(row.strategy_id, row.factory_status) for row in plan] == [
        ...
        ("MNQ_NYSE_OPEN_E2_RR1.0_CB1_NO_FILTER_O15_S075", "BLOCKED_NON_DEFAULT_STOP"),
    ]
    assert plan[1].draft_path is None
    assert "outcome_builder rebuild" in plan[1].next_action
...
def test_write_factory_artifacts_emits_batch_files(tmp_path: Path) -> None:
    ...
    assert "MNQ_NYSE_OPEN_E2_RR1.0_CB1_NO_FILTER_O15_S075" not in commands
