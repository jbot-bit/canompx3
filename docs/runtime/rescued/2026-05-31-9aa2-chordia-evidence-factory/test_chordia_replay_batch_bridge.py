...
def test_plan_acceptance_uses_only_ready_rows(tmp_path: Path) -> None:
    ...
    assert [(row.strategy_id, row.bridge_status) for row in plan] == [
        ...
        ("MNQ_NYSE_OPEN_E2_RR1.0_CB1_NO_FILTER_O15_S075", "SKIP_FACTORY_STATUS"),
    ]
    assert plan[0].active_hypothesis_path.endswith("2026-05-31-mnq-nyse-open-e2-rr1-0-cb1-no-filter-o15-chordia-unlock-v1.yaml")
    assert plan[1].active_hypothesis_path == ""
...
def test_write_bridge_artifacts_outputs_plan_and_no_audit_mutation(tmp_path: Path) -> None:
    ...
    bridge.write_bridge_artifacts(plan, output_dir=output_dir, results=())
