...
def plan_acceptance(
    *,
    ...
    for row in _read_batch(factory_dir, batch_id):
        ...
        source_draft = repo_root / source_draft_text if not Path(source_draft_text).is_absolute() else Path(source_draft_text)
        ...
        rows.append(
            BridgePlanRow(
                ...
                result_md_path=_artifact_rel(result_path, repo_root=repo_root) if status != "BLOCKED_MISSING_DRAFT" else "",
...
def write_bridge_artifacts(
    ...
    *,
    ...
    proposals = build_audit_log_proposals(parsed_results, audit_date=datetime.now(UTC).date().isoformat())
