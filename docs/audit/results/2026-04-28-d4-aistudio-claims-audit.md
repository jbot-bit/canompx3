...
> **CAVEAT — added 2026-04-29 (E2 look-ahead bias on selector + clean re-derivation).** The numeric claims 4–12 below were generated by `research/audit_aistudio_claims_d4.py`, which filters every numeric query on `daily_features.orb_COMEX_SETTLE_break_dir='long'`. On E2 entries the canonical `break_dir` (close-cross at the break bar) is post-entry on ~42% of fills (range-touch fakeouts that resolve the other way), same class bug as registry row 27 `shadow_htf_mes_europe_flow_long_skip.py`. Script annotated `# e2-lookahead-policy: tainted` 2026-04-29; CRG `importers_of`=0, zero deployment-path consumers.
>
> **Clean re-derivation, IS only (`trading_day < HOLDOUT_SACRED_FROM`), `MNQ COMEX_SETTLE O5 E2 CB1 RR1.0` cohort, no `break_dir` filter, `orb_COMEX_SETTLE_size>=5` (= ORB_G5):**
>
> | Test | Original (tainted, N=839) | Clean (N=1577) | Survives? |
> |---|---|---|---|
> | Claim 6 — `garch>70` t-stat | 3.03 | 2.91 | YES (dir same) |
> | Claim 6 — `atr_20>70` t-stat | 2.60 | 3.34 | **DIRECTION WRONG** — clean ATR > clean GARCH |
> | Claim 6 — `ovn_range>80` t-stat | 1.74 | 2.34 | YES (dir same; magnitude inflated under cleaner data, not deflated) |
> | Claim 6 ranking | garch > atr > ovn | **atr > garch > ovn** | **REVERSES (with confound — see note)** |
> | Claim 7 — Q5 mean-R | +0.284 | +0.238 | YES (Q5 dominance survives) |
> | Claim 7 — Q3 "dead zone" mean-R | -0.037 | +0.008 | **NO** — Q3 dead-zone is selector artifact |
>
> **Confound note (added 2026-04-29 self-review).** The "Clean (N=1577)" cohort is *both-sides* on the lane (no `break_dir` filter), while "Original (tainted, N=839)" is *long-only* via `break_dir='long'`. The N=839 → N=1577 jump combines two independent effects: (a) removing the look-ahead selector, (b) adding short-side trades. The Claim 6 ranking reversal therefore cannot be cleanly attributed to selector bias alone — it could equally reflect the long/short asymmetry of `atr_20_pct` vs `garch_forecast_vol_pct` on this lane. To isolate selector contamination from direction mix would require a long-only-with-pre-break-direction-proxy re-run (e.g. `gap_open_points > 0` as a clean direction proxy) compared to the long-only-tainted N=839 figures. That clean-proxy re-run was not performed in this caveat pass. **What the table proves unambiguously:** the original N=839 long-only cohort was conditioned on a post-entry selector. **What it does not prove:** that selector bias alone causes the ranking flip. The Claim 6 RETRACT verdict below stands on the conservative reading (selector-tainted evidence cannot support the "GARCH dominates" claim regardless of which mechanism explains the clean ranking) but the literal "clean ranking is atr > garch" framing requires a long-only-with-clean-proxy run before being cited as deployment evidence.
>
> **Verdict updates:**
>
> - **Claim 1 (GARCH timing)** — survives. Doctrinal, no data dependence.
> - **Claim 4 (fixed-dollar slippage)** — survives. Conclusion rests on `pipeline/cost_model.py` MNQ slippage=$1.00 fixed; the biased-subset evidence is consistent with that source-code fact.
> - **Claim 6 (alt-filter ranking)** — **RETRACT.** Clean ranking is `atr_20_pct > garch > ovn_range`; original conclusion "GARCH dominates" is selector artifact. Future Phase D pre-regs evaluating pre-break vol predictors on this lane should treat `atr_20_pct` as the leading candidate, not GARCH.
> - **Claim 7 (Q5 dominance + Q3 dead zone)** — **PARTIAL RETRACT.** Q5 dominance survives (+0.238 R clean, ≈3-7× other quintiles). Q3 dead-zone framing dissolves on clean re-run. Headline "Q5 alone" remains correct.
> - **Claim 12 (aperture deltas)** — not re-derived in this caveat pass; the same selector bias applies. Direction (5m strongest) likely survives but magnitudes inflated. Re-derive before citing as deployment evidence.
> - **Claim 8 (momentum confirmed)** — uses MFE/MAE on the same biased subset. Re-derive before citing.
>
> **D4 PARK_PENDING_OOS_POWER verdict (`docs/audit/results/2026-04-28-mnq-comex-settle-pathway-b-v1-result.md`):** survives — Q5 garch>70 edge holds clean (+0.238 R, t=2.91, p=0.0037 on N=370/1207 split). PARK is correct. Re-pre-registration of D4 alt-predictor candidate should use `atr_20_pct` per clean Claim 6 finding.
