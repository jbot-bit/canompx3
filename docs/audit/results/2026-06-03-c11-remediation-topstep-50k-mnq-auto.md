...
What this audit tests: whether `topstep_50k_mnq_auto` clears canonical Criterion 11
(account survival) for funded deployment, and if not, what remediation space exists.
Both C11 sub-gates are in scope — (a) Monte Carlo 90-day operational survival ≥70%,
and (b) strict observed 90-day drawdown ≤ 80% of MLL ($1,600) with zero historical
daily-loss breach days. Out of scope: live order-path correctness (covered by the
bracket-risk-parity closeout) and the deployable cap value itself (none is locked).
...
- Command to reproduce the C11 verdict: `python -m trading_app.account_survival --profile topstep_50k_mnq_auto --no-write-state`.
- Canonical DB read (read-only): `C:\Users\joshd\canompx3\gold.db`.
- Companion artefacts: `docs/audit/results/2026-06-03-c11-attribution-closeout.md` (attribution lock), `docs/audit/results/2026-06-03-plan-v2-current-execution.md` (point-in-time execution record), `docs/runtime/stages/c11-80pt-cap-wiring.md` (corrected remediation stage).
- Sensitivity figures cited above (uncapped vs cap-emulated at 0.75 / 0.50 stop) are reproduced from the `## Sensitivity summary` section's runtime-cap-emulation cells.
- Replay-parity plumbing landed at `9b3fc530` (origin/main); preserve branch `c11-orb-cap-preserve-2026-06-04` (`48071955`).
