...
A point-in-time execution record of the Plan v2 live-readiness pass against the
current Windows working tree (NOT clean `origin/main`). The question: what blockers
stand between `topstep_50k_mnq_auto` and a deployable state, as measured at this
checkout. NOTE: the git-state figures recorded here (`main` at `7315ccbc`, "ahead 3 /
behind 9") are STALE as of commit-time — HEAD is now `9b3fc530`, 0 ahead / 0 behind
origin/main. This file is committed as a dated audit record, not as current truth.
...
Plan v2 executed properly as a blocker taxonomy; live promotion remains BLOCKED.
C11 is the binding blocker; allocation ranking is the wrong next move while C11 fails.
No deployment is supported. The proper next step is lock-aware attribution/C11
remediation in an isolated session, not more strategy discovery.

## Outputs

Read-only commands run this pass (reproduce against the same checkout):
...
- `python scripts/tools/workflow_doctor.py status --json` / `... db --json`
Companion: `docs/audit/results/2026-06-03-c11-remediation-topstep-50k-mnq-auto.md`,
