...
| 28 | `audit_aistudio_claims_d4.py` | `break_dir='long'` as WHERE-clause selector across claims 5–12 (8 query sites; same class as row 27) | TAINTED (added 2026-04-29 by drift-check sweep) | `# e2-lookahead-policy: tainted` — audit-of-audit script generating `2026-04-28-d4-aistudio-claims-audit.md`. Bias direction: excludes fakeout-then-reverse trades, inflating t-stats / mean-R on the deployed lane (`MNQ COMEX_SETTLE O5 E2 CB1 RR1.0 long ORB_G5`). The D4 PARK_PENDING_OOS_POWER verdict supported by these numbers is more permissive than a clean re-run would be. CRG `importers_of`=0, zero deployment-path consumer. Audit-result doc carries inline caveat (commit 2026-04-29). Claims 1, 4, 14 (non-numeric / direction-independent) survive; claims 6, 7, 12 magnitude-dependent require re-derivation. |
| 29 | `phase_d_d0_v2_backtest.py` | none — `rel_vol` / `break_bar` only appear in docstring text explaining what the v2 supersedes | CLEARED (annotation typo correction 2026-04-29) | `# e2-lookahead-policy: cleared` — was previously annotated `# e2-lookahead-policy: clean` (one-letter contract violation), drift check re-flagged each commit. Genuinely clean: predictor is `garch_forecast_vol_pct` (RULE 6.1 safe-list, prior-day close, pre-entry). Pre-reg `2026-04-28-phase-d-d0-v2-garch-clean-rederivation.yaml` SHA `823b0127`. |
| 30 | `chordia_strict_unlock_v1.py` (PR #211 audit `MNQ_COMEX_SETTLE_E2_RR1.0_CB1_X_MES_ATR60`) | `cross_atr_MES_pct` — derived from `daily_features.atr_20_pct` of source instrument MES (rolling, prior-close, `resolves_at="STARTUP"`); not break-bar state. No `rel_vol`, `break_bar*`, `break_delay`, or `break_dir` references in the runner's predictor path. Filter `X_MES_ATR60` not caught by `E2_EXCLUDED_FILTER_PREFIXES = ("VOL_RV", "ATR70_VOL")` or `E2_EXCLUDED_FILTER_SUBSTRINGS = ("_CONT", "_FAST", "NOMON_CONT")` (`trading_app/config.py:3904-3912`). | CLEARED (added 2026-05-04 by post-merge evidence-auditor) | `# e2-lookahead-policy: cleared` — `X_MES_ATR60` is the 60th-percentile ATR threshold on prior-close MES atr_20_pct, not a 60-period ATR window. Naming asymmetry verified, no temporal contamination. Audit-row in `chordia_audit_log.yaml` includes the derivation in its `note` block. PR #211 (merged `7b5b6e49`) audit verdict PASS_CHORDIA stands. |

## Drift-check learnings — 2026-04-29 sweep

Two candidates surfaced by `check_e2_lookahead_research_contamination()` after the 18+9 registry was frozen:
- **One real find (row 28)** — the `orb_\w+_break_dir` regex addition (2026-04-28) caught a structural class-bug copy on a script the original sweep missed. Confirms the drift-check pattern extension paid off.
- **One annotation typo (row 29)** — `clean` vs `cleared` is a one-letter contract violation. Future hardening considered: extend `valid_markers` in `check_e2_lookahead_research_contamination` to accept `clean` as an alias for `cleared`, OR keep the strict contract and rely on drift-check re-firing as the forcing function. Decision deferred — strict contract is currently surfacing the problem in <1 commit cycle, so no change needed.

CRG (code-review-graph) `importers_of` query was load-bearing for the row 28 verdict: zero deployment-path consumers reduced action from "refactor + re-run on clean cohort" to "annotate + audit-doc caveat."

## Future-proofing follow-up (deferred — not landed 2026-04-29)

Considered but not landed because edits to `pipeline/check_drift.py` are stage-gate core and require explicit per-edit approval:

1. **Drift check: tainted-script-cited-in-audit-doc consistency.** A new `check_e2_lookahead_audit_doc_caveat()` would scan `docs/audit/results/*.md` for filename references to scripts in the TAINTED registry (rows 2-18 pre-2026-04-28 + rows 19-28 post-sweep) and require an inline caveat citing this registry. Catches the failure mode where a future audit-result doc cites a tainted script's number without acknowledging the bias.
2. **Annotation typo defense (decided NO).** Aliasing `clean` → `cleared` in `valid_markers` was considered and rejected. Strict contract is currently surfacing typos within one commit cycle; silent alias would weaken the semantic distinction between `cleared` (verified clean) and `not-predictor` (audited feature, not used as predictor). Keep strict contract; rely on drift-check re-firing as the forcing function.
3. **Postmortem cross-check.** The 2026-04-21 postmortem § 5.1 listed 18 candidate scripts; the 2026-04-28 sweep added 9; the 2026-04-29 sweep added 2 (rows 28-29); the 2026-05-04 post-merge evidence-auditor added 1 (row 30). Cumulative tally across 30 reviewed scripts: TAINTED=24 (rows 2-17, 19-24, 27, 28), FIXED=1 (row 1, post-fix canonical), LIKELY CLEAN — verify pending=1 (row 18), CLEARED=3 (rows 25, 29, 30), NOT-PREDICTOR=1 (row 26). Future drift-check pattern extensions (e.g., a new `daily_features` column added with ambiguous temporal semantics) should add to the regex AND trigger a registry sweep run.
