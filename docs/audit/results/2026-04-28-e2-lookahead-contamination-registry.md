...
3. **Postmortem cross-check.** The 2026-04-21 postmortem § 5.1 listed 18 candidate scripts; the 2026-04-28 sweep added 9; the 2026-04-29 sweep added 2 (rows 28-29). Cumulative tally across 29 reviewed scripts: TAINTED=24 (rows 2-17, 19-24, 27, 28), FIXED=1 (row 1, post-fix canonical), LIKELY CLEAN — verify pending=1 (row 18), CLEARED=2 (rows 25, 29), NOT-PREDICTOR=1 (row 26). Future drift-check pattern extensions (e.g., a new `daily_features` column added with ambiguous temporal semantics) should add to the regex AND trigger a registry sweep run.
