...
|---|---|---|---|---|---|
| `COST_LT12` gate | `daily_features` / canonical filter registry | `daily_features.orb_NYSE_OPEN_size + pipeline.cost_model.COST_SPECS` | `ORB_FORMATION` | CLEAR | `Phase A A4 marked COST_LT12 timing-valid for the live six-lane book.` |
| max_orb_size_pts overlay | prop_profiles + daily_features | `orb_NYSE_OPEN_size` with cap `117.8` | ORB_FORMATION | CLEAR | Phase A snapshot 5e768af8 separated the execution overlay and measured the cap pass-rate for this lane. |
