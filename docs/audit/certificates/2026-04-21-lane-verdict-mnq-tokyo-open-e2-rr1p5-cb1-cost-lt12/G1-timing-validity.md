...
This retrospective lane verdict reuses the binding timing-validity result from [the Phase A snapshot](/mnt/c/Users/joshd/canompx3/docs/audit/2026-04-21-reset-snapshot.md:615). The lane uses `COST_LT12` only; no rel_vol or break-bar variables are present.
...
|---|---|---|---|---|---|
| `COST_LT12` gate | `daily_features` / canonical filter registry | `daily_features.orb_TOKYO_OPEN_size + pipeline.cost_model.COST_SPECS` | `ORB_FORMATION` | CLEAR | `Phase A A4 marked COST_LT12 timing-valid for the live six-lane book.` |
| max_orb_size_pts overlay | prop_profiles + daily_features | `orb_TOKYO_OPEN_size` with cap `45.6` | ORB_FORMATION | CLEAR | Phase A snapshot 5e768af8 separated the execution overlay and measured the cap pass-rate for this lane. |
