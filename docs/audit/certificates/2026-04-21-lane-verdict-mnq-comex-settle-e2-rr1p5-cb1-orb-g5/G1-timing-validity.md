...
|---|---|---|---|---|---|
| `ORB_G5` gate | `daily_features` / canonical filter registry | `daily_features.orb_COMEX_SETTLE_size` | `ORB_FORMATION` | CLEAR | `Phase A A4 marked ORB_G5 timing-valid for the live six-lane book.` |
| max_orb_size_pts overlay | prop_profiles + daily_features | `orb_COMEX_SETTLE_size` with cap `52.8` | ORB_FORMATION | CLEAR | Phase A snapshot 5e768af8 separated the execution overlay and measured the cap pass-rate for this lane. |
