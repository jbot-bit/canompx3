...
"""
Session scheduling, filter translation, briefing card builder, and UI state machine.
...
Extended from ui/session_helpers.py with:
- StateName enum for typed state transitions
- SessionState dataclass for multi-session overlap (session stack)
- resolve_global_state() priority function
- ET time helper for display
"""
...
@dataclass
class SessionState:
    """State for a single active session in the session stack."""

    session_name: str
    instrument: str | None = None
    sub_state: StateName = StateName.ORB_FORMING
    orb_start: datetime | None = None
    orb_minutes: int = 5
...
_STATE_PRIORITY = {
    StateName.DEBRIEF: 6,
    StateName.IN_SESSION: 5,
    StateName.ORB_FORMING: 4,
    StateName.ALERT: 3,
    StateName.APPROACHING: 2,
    StateName.IDLE: 1,
    StateName.OVERNIGHT: 0,
    StateName.WEEKEND: 0,
}


def resolve_global_state(base_state: AppState) -> StateName:
    """Resolve global UI state from base clock state + session stack.

    Global state = highest priority among base state and active sessions.
    """
    best = _STATE_PRIORITY.get(base_state.name, 0)
    best_name = base_state.name

    for session in base_state.active_sessions:
        p = _STATE_PRIORITY.get(session.sub_state, 0)
        if p > best:
            best = p
            best_name = session.sub_state
...
def get_app_state(now: datetime) -> AppState:
    ...
    Clock-based states (returned by this function):
    ...
    Event-driven states (pushed externally via session stack):
      ORB_FORMING -- ORB aperture window active
      IN_SESSION  -- Trade is live, monitoring
      DEBRIEF     -- Session complete, reviewing outcome
    """
    ...
    if not upcoming:
        ...
        return AppState(name=StateName.IDLE, trading_day=trading_day)
    ...
    base = AppState(
        ...
        trading_day=trading_day,
        name=StateName.IDLE,  # default, refined below
    )

    # Determine state
    if not _is_in_awake_hours(next_dt) and minutes_to_next > 60:
        base.name = StateName.OVERNIGHT
    elif minutes_to_next <= 15:
        base.name = StateName.ALERT
    elif minutes_to_next <= 60:
        base.name = StateName.APPROACHING
    else:
        base.name = StateName.IDLE
