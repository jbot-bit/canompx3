...
| G6+ | ORB >= 6 points | MGC: strongest per-trade edge, fewer trades. MNQ: no-op. |
...
| VWAP_BP_ALIGNED on MNQ CME_PRECLOSE O5 | NO-GO. Why: OOS direction flipped post-2026-01-01 on all 3 RR cells (IS t=3.04-5.35 → OOS t=−0.88 to −2.39), fails Amendment 2.7 Criterion 8. Result doc: `docs/audit/results/2026-04-18-vwap-bp-cmepreclose-c8-recheck.md`. Commit `284e3d35`. Parent VWAP family (US_DATA_1000 O15 VWAP_MID_ALIGNED) unaffected. Apr 2026. |
| MNQ WIDE_REL_1.0X_20D × ORB_G5 at O5 | NO-GO. Why: K=6 pre-reg replay 0/6 pass primary; all cells fail Chordia t ≥ 3.79 (max t=2.74). Rule 7 fire correlation distinct (0.13-0.55), so not tautological with G5 — just not strong enough. Result doc: `docs/audit/results/2026-04-18-mnq-wide-rel-ib-v2-replay.md`. Commit `e1cc2977`. Apr 2026. |
| CROSS_NYSE_MOMENTUM on MNQ US_DATA_1000 + NYSE_CLOSE at O5 | NO-GO. Why: K=6 pre-reg replay 0/6 pass primary; all fail Chordia t ≥ 3.79 (max t=2.15). US_DATA_1000 3/3 cells tripped LEAKAGE_SUSPECT upper guard (eff_ratio 3.5-287× from near-zero IS baselines). NYSE_CLOSE 3/3 cells N_OOS<30. Apr 11 memo's IS +0.140 "strongest branch" claim did not reproduce (current RR1.0 IS +0.0006). Result doc: `docs/audit/results/2026-04-18-mnq-cross-nyse-momentum-replay.md`. Commit `16cc3af5`. Apr 2026. |
| OVNRNG_100 on MNQ NYSE_OPEN + US_DATA_1000 at O5 | NO-GO. Why: tested 2026-04-11 and REJECTED through validator pipeline. NYSE_OPEN fails Phase 3 year stability + raw p>0.10. US_DATA_1000 RR1.0 passes IS BH FDR but rejects Criterion 8 (OOS ExpR −0.009 N=61). Result doc: `docs/audit/results/2026-04-18-overnight-adjacent-family-status.md`. Commit `66d26edb`. Apr 2026. |
...
| Idea | Status | Notes |
|------|--------|-------|
| **Time-based exit gate** | **NO-GO (T80 checkpoint)** | T80 time-stop ruled NO-GO on OOS paired portfolio replay — did not survive walk-forward (MNQ delta=-0.008 p=0.02 HURTS; MGC/MES inconclusive). See `trading_app/config.py:4257-4271` (`EARLY_EXIT_MINUTES`, all sessions `None`) and `scripts/research/test_t80_oos.py`. Do NOT re-litigate the T80 checkpoint. Winner-speed in-sample story (T80 <= 45m at RR1.0; 90-170m at RR2.5-3.0; 300-450m dead exposure) did not generalize OOS. **Caveat:** the NO-GO covers the T80 checkpoint specifically; a distinct T120/T180 dead-winner variant remains formally untested (low prior given T80 hurt — any pursuit must clear the same OOS + pre-registered K-budget bar). Script: `research/research_winner_speed.py`. |
