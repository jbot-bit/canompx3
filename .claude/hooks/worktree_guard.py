...
def _emit_block(peer_lease: dict | None, lease_module, cwd: Path) -> None:
    ...
    lines = [
        ...
        "",
        "  Safe next action (pick one):",
        "    1. Work in your OWN worktree (this block means you are sharing one):",
        "         scripts/tools/new_session.sh <descriptor>",
        "    2. If the peer is CONFIRMED gone (LIVE above means it is NOT gone),",
        "       force-release the lease and retry:",
        "         python scripts/tools/worktree_guard.py --force-release",
        "    3. Override for this session only (set in the harness env, then",
