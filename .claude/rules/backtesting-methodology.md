...
4. For underpowered OOS situations, prefer one of:
   ...
   - **CPCV** (Combinatorial Purged Cross-Validation) — designed for short OOS data; binary IS/OOS split is misspecified when OOS < ~20% of total sample. **SANCTIONED but DOCTRINE-ONLY / NOT-YET-IMPLEMENTED (as of 2026-05-31): no CPCV implementation exists in the repo and no declared pre-reg schema for it. A pre-reg may NOT cite CPCV as a validation gate until it is built.** When implemented it is a research-validation path only (same tier as DSR-fixed-universe per `docs/institutional/pre_registered_criteria.md` Criterion 8), NOT deployment proof — forward-OOS remains required for deployment. Primary source: AFML 2018 Ch 12 § 12.4 (`docs/institutional/literature/lopez_de_prado_2018_afml_ch_3_7_8.md`). Secondary summary: LdP 2020 § 1.4.2 (`docs/institutional/literature/lopez_de_prado_2020_ml_for_asset_managers.md`). The 2020 sampler describes CPCV's use case; the 2018 book formalizes the algorithm.
