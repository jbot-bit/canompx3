...
### 4.3 Per-cell significance requirement for classification thresholds (added 2026-04-20)

**Any classification / labeller / pattern-assignment step that uses a bare numeric threshold on a cell-level metric (e.g. |delta| > 0.03, wr_spread > 5%, lift > 0.10) MUST also compute the per-cell p-value and report it alongside the classification.** Labels may NOT be elevated above what per-cell significance supports.

Minimum required columns on any such classifier output:
- `label_raw` — the bare-threshold classification
- `year_t` or equivalent per-cell subset-t
- `year_p` — two-sided p-value from the t-distribution at df = n-1
- A BH-FDR-conditioned `label_confirmed` column that downgrades `label_raw` to `_unconfirmed` when p-value fails the FDR gate at the appropriate K framing

**Why this rule exists.** The Phase 2.8 v1 result doc (`docs/audit/results/2026-04-19-multi-year-regime-stratification.md`, superseded 2026-04-19 by `docs/audit/results/2026-04-19-phase-2-9-comprehensive-multi-year.md`) used a classifier that flagged `SINGLE_YEAR_DRAG` whenever `|delta| > 0.03 AND n >= 30 AND year_expr <= -0.05`. No per-cell significance test. 3 cells received the label. Under honest per-cell t-test at K_global=266 / K_session / K_year framings, all 3 cells have |t| < 1.1 and two-sided p > 0.28 — well above any BH critical value. The bare-threshold label elevated noise to "significant drag" in the result narrative. Retirement verdicts stood independently (Phase 2.4 / 2.7 cross-confirmed), but v1's Phase 2.8 statistical case for those retirements was unsupported.

**Reference implementation (2026-04-20):** `research/phase_2_9_comprehensive_multi_year_stratification.py::assign_v2_pattern` returns `{DRAG,BOOST}_confirmed` only when the cell survives BH-FDR at K_session OR K_year; otherwise downgrades to `{DRAG,BOOST}_unconfirmed`. Bare `DRAG_candidate` / `BOOST_candidate` labels exist transiently in an intermediate `label_raw` column before this downgrade.

**Grounding.** Harvey-Liu 2015 (`docs/institutional/literature/harvey_liu_2015_backtesting.md` p17): "an OOS test's success can be due to luck for both the in-sample selection and the out-of-sample testing." Bare-threshold labels ARE in-sample selections that don't discount luck. Benjamini-Hochberg 1995 (`docs/institutional/literature/benjamini_hochberg_1995_fdr.md`) provides the canonical FDR-controlling procedure to apply as the gate.

**What this rule forbids.**
- PATTERN columns / verdict labels derived from `|delta| > threshold` alone without a companion `p` column.
- Narrative claims like "14 lanes show drag" / "34 lanes VOL_NEUTRAL" sourced from bare-threshold classifiers.
- "SINGLE_YEAR_DRAG" / "BOOST" / "RECURRING_REGIME" labels applied without BH-FDR confirmation at the promotion framing.

**What this rule allows (transiently).** A `label_raw` / `label_candidate` column produced by a bare-threshold pre-filter IS acceptable as an intermediate representation, provided the final classifier output includes the per-cell p-value and the BH-conditioned confirmed label.
...
- **2026-04-19: Bare-threshold classifier labels elevated noise to 'significant drag' (new class, RULE 4 sub-clause 4.3).** The Phase 2.8 v1 multi-year stratification result doc (`docs/audit/results/2026-04-19-multi-year-regime-stratification.md`) used a classifier that flagged `SINGLE_YEAR_DRAG` when `|delta| > 0.03 AND n >= 30 AND year_expr <= -0.05`. No per-cell significance test. 3 cells received the label (2 SGP CROSS_SGP_MOMENTUM × 2024, 1 X_MES_ATR60 × 2020). Honest per-cell t-test at K_global=266 / K_session / K_year BH-FDR (via `research/phase_2_9_comprehensive_multi_year_stratification.py`) showed all 3 cells have |t| 0.58 to 1.08 and two-sided p 0.28 to 0.57 — well above any BH critical value at any K framing. The bare-threshold label elevated noise to "significant drag" in the result narrative. Retirement verdicts stood independently (Phase 2.4/2.7 confirmed), but v1's Phase 2.8 statistical case was unsupported. **Lesson codified as RULE 4.3 (added 2026-04-20):** any classifier using a bare numeric threshold on a cell-level metric MUST also compute + report the per-cell p-value, and confirmed labels MUST be downgraded to `_unconfirmed` when BH-FDR fails at the appropriate K framing. Reference implementation: `research/phase_2_9_comprehensive_multi_year_stratification.py::assign_v2_pattern`.
