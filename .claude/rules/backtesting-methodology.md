...
- **2026-04-20: Re-audited non-deployed lanes — portfolio-relevance check missing.** Block-bootstrap null re-audit (`research/phase-2-9-xmes-block-bootstrap-reaudit`) validated 4 phase-2-9 framing-audit lanes (OVNRNG_100 x2, X_MES_ATR60 x2 on MNQ COMEX_SETTLE) at p<0.05 per lane. Result is methodologically correct. At end-of-session, queried `docs/runtime/lane_allocation.json` for the first time and found **NONE of the 4 audited lanes are in the live 2026-04-18 allocator DEPLOY set** — the allocator chose ORB_G5 on COMEX_SETTLE instead. The entire session validated a null on a portfolio-irrelevant lane set. Research-provisional promotion (2026-04-11) does not imply deployment; the allocator had already overridden it a week later. **Lesson / RULE 14 candidate:** at the start of any session claiming to "re-audit" / "validate" / "confirm" a specific promoted lane, query `docs/runtime/lane_allocation.json` and confirm the lane is in the current DEPLOY list. If not, either (a) label the audit as academic / non-portfolio-actionable explicitly in the pre-reg, or (b) pivot to a deployed lane with the same research question. Handoff at `docs/handoffs/2026-04-20-xmes-reaudit-session-handoff.md`. See tunnel-vision memory entry `memory/phase_2_9_xmes_reaudit_tunnel_lesson.md`.
